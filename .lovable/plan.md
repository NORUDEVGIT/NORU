...
Once the NORU logo is uploaded, store it as a CDN asset pointer and add a small `NoruLogo` component with size variants:

- desktop sidebar (dark background treatment)
- mobile header (compact mark)
- auth pages (larger, centred)
- public/customer header (restraint — the restaurant's own name stays dominant)

Aspect ratio preserved, transparency preserved, no redrawing. Favicon generated by downscaling the same logo to a square `public/favicon.png`, referenced from `__root.tsx`, and the default `favicon.ico` removed.
