...
import { StatusBadge, VipBadge } from "@/routes/restaurant/guests/index";
...
export const Route = createFileRoute("/restaurant/guests/$guestId")({
  ...
  head: () => ({
    meta: [
      { title: "Guest profile — NORU" },
      {
        name: "description",
        content: "Guest overview, preferences and profile history for your NORU property.",
      },
      { property: "og:title", content: "Guest profile — NORU" },
...
function GuestDetailRoute() {
  return <RestaurantShell active="Guests">{(m) => <GuestDetail membership={m} />}</RestaurantShell>;
}
...
function GuestDetail({ membership }: { membership: RestaurantMembership }) {
  ...
  if (guestQuery.isError || !guestQuery.data) {
    return (
      <div className="rounded-2xl border border-border bg-card p-6">
        ...
        <Button className="mt-4" variant="outline" onClick={() => void navigate({ to: "/restaurant/guests" })}>
          Back to guests
        </Button>
      </div>
    );
  ...
  return (
    <div className="space-y-6">
      <button
        type="button"
        onClick={() => void navigate({ to: "/restaurant/guests" })}
