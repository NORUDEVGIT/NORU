...
export const Route = createFileRoute("/restaurant/bookings/reservations")({
  ...
  beforeLoad: async () => {
    ...
    if (error || !data.user) {
      throw redirect({
        to: "/restaurant/login",
        search: { redirect: "/restaurant/bookings/reservations" },
      });
    }
  },
  head: () => ({
    meta: [
      ...
      { property: "og:title", content: "Reservations — NORU" },
      { property: "og:description", content: "Every reservation for your property in one list." },
...
function ReservationsRoute() {
  return <RestaurantShell active="Reservations">{(m) => <ReservationsPage membership={m} />}</RestaurantShell>;
}

const ALL = "all";
const PAGE_SIZE = 25;

function ReservationsPage({ membership }: { membership: RestaurantMembership }) {
