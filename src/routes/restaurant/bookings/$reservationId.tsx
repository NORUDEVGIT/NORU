...
export const Route = createFileRoute("/restaurant/bookings/$reservationId")({
  ...
  head: () => ({
    meta: [
      { title: "Reservation — Booking & Guest Management — NORU" },
      {
        name: "description",
        content: "Reservation detail: stay dates, room assignment, status actions and full change history.",
      },
      { property: "og:title", content: "Reservation — NORU" },
...
function ReservationDetailRoute() {
  ...
  return (
    <RestaurantShell active="Reservations">
      {(m) => <ReservationDetailPage membership={m} reservationId={reservationId} />}
    </RestaurantShell>
  );
}
    ...
    return (
      <div className="rounded-2xl border border-border bg-card p-6">
        ...
        <Button asChild className="mt-4" variant="outline">
          <Link to="/restaurant/bookings/reservations">Back to reservations</Link>
        </Button>
      </div>
    );
  }
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        ...
        <section className="rounded-2xl border border-border bg-card p-4">
          ...
          <Button asChild variant="outline" size="sm" className="mt-4">
            <Link to="/restaurant/guests/$guestId" params={{ guestId: reservation.guestId }}>
              Open guest profile
            </Link>
          </Button>
        </section>
