...
export function GuestCompanyGuestLinks({
  ...
  const guestsQuery = useQuery({
    queryKey: ["guests-pick", restaurantId, search, "company-multi"],
    queryFn: () =>
      fetchGuests({
        data: {
          restaurantId,
  ...
  return (
    <div className="space-y-3 rounded-2xl border border-border bg-card p-4" data-testid="company-guest-links">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <p className="text-xs uppercase tracking-wide text-muted-foreground">Linked guests</p>
          <p className="mt-1 text-sm text-muted-foreground">{COMPANY_MULTI_LINK_COPY}</p>
        </div>
        <Button
          variant="outline"
      ...
      {pickerOpen ? (
        <div className="grid gap-3 rounded-xl border border-dashed border-border p-3" data-testid="company-link-picker">
          <div className="grid gap-3 sm:grid-cols-2">
            <Select value={role} onValueChange={(value) => setRole(value as CompanyLinkRole)}>
              <SelectTrigger data-testid="company-link-role">
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
