...
import { BulkRestrictionProgress, type BulkRestrictionStep } from "./bulk-restriction-progress";
      ...
      <div className="flex items-start justify-between gap-3 border-b border-[#E8E1D7] px-4 py-3">
        <div>
          ...
          <h3 className="mt-1 font-display text-lg font-semibold text-[#251605]">
            Update operational restrictions
          </h3>
        </div>
        {onClose ? (
          <button
...
export function BulkRestrictionPanel({
  step,
  progress,
  body,
  footer,
  ...
  const chrome = (
    <PanelChrome onClose={() => setOpen(false)} footer={footer}>
      {progress ? <BulkRestrictionProgress step={step} /> : null}
      {body}
    </PanelChrome>
  );
