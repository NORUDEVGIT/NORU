...
import type { RevenueControlSignal, RevenueControlWorkRow } from "@/packages/pms/lib/revenue/revenue-control";

function signalTone(signal: RevenueControlSignal): "neutral" | "warning" | "danger" | "info" {
  if (signal === "Restriction Active") return "danger";
  if (signal === "Low Remaining Inventory" || signal === "High Occupancy") return "warning";
  ...
  return (
    <section className="rounded-xl border border-[#E8E1D7] bg-card shadow-sm">
      <div className="border-b border-[#E8E1D7] px-3 py-2.5">
        <h2 className="text-sm font-semibold text-[#251605]">Rate & Inventory Control</h2>
        <p className="text-[10px] text-muted-foreground">
          Nightly booked occupancy with the focused rate plan. No recommended percentage changes.
        </p>
      </div>
      {rows.length === 0 ? (
        <p className="px-3 py-6 text-xs text-muted-foreground">No dates in the selected range.</p>
      ) : (
          ...
          <table className="w-full min-w-[860px] text-left text-[10px]">
            ...
            <tbody>
              {rows.map((row) => {
                ...
                return (
                  <tr key={row.date} className="border-b border-[#E8E1D7]/80 hover:bg-muted/20">
                    <td className="px-3 py-2 font-medium text-[#251605]">{row.date}</td>
                    <td className="px-3 py-2">{row.occupancyPercent}%</td>
                    <td className="px-3 py-2">{row.soldRoomNights}</td>
                    <td className="px-3 py-2">{row.availableRoomNights}</td>
                    <td className="px-3 py-2">
                      {row.effectiveRate == null
                        ? "—"
                        : `${row.ratePlanCode ?? "Rate"} ${money(row.effectiveRate)}${row.overrideActive ? " · override" : ""}`}
                    </td>
                    <td className="px-3 py-2">{row.restrictionLabel ?? "—"}</td>
                    <td className="px-3 py-2">
                      <InventoryStatusBadge tone={signalTone(row.signal)}>{row.signal}</InventoryStatusBadge>
                    </td>
                    <td className="px-3 py-2">
