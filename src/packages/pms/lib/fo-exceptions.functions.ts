...
export async function loadFoStaySignals(params: {
  context: Parameters<typeof requireReservationManager>[0];
  restaurantId: string;
  reservationIds?: string[];
}): Promise<FoStaySignalsResult> {
  const { context, restaurantId } = params;
  const byStay: Record<string, StayMoneySignal> = {};
  const ids = params.reservationIds ?? [];

  if (ids.length === 0) {
    try {
      await requireCashieringAccess(context as never, restaurantId);
  ...
  if (!progressError) {
    for (const row of (progressRows ?? []) as Array<{
      ...
      signal.keyIssued = Boolean(
        (row.key_access_type ?? "").trim() && (row.key_identifier ?? "").trim(),
      );
...
export async function loadFoExceptionFeeds(params: {
  context: Parameters<typeof requireReservationManager>[0];
  restaurantId: string;
  businessDate: string;
  canResolveDiscrepancy: boolean;
}): Promise<FoExceptionFeedsResult> {
  const { context, restaurantId, businessDate, canResolveDiscrepancy } = params;
  const endDate = addDays(businessDate, OVERBOOKING_HORIZON_DAYS - 1);
