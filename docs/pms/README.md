...
## Guest Profile Module — Waves 1–2 on main; Waves 3–5 still gated

| Record | Classification |
|---|---|
| [Guest Profile Module — programme overview](./guests.md) | **Functional Spec / programme** — Waves 1–2 **IMPLEMENTED ON MAIN**. Wave 1: issue [#66](https://github.com/NORUDEVGIT/NORU/issues/66) CLOSED · PR [#67](https://github.com/NORUDEVGIT/NORU/pull/67) MERGED. Wave 2: issue [#72](https://github.com/NORUDEVGIT/NORU/issues/72) CLOSED · PR [#76](https://github.com/NORUDEVGIT/NORU/pull/76) MERGED · PR [#79](https://github.com/NORUDEVGIT/NORU/pull/79) MERGED. Waves 3–5 still **WAVE-GATED**. Module is **not** COMPLETE. |
| [Functional Spec — Guest Profile Module](./specs/guest-profile-module.md) | **Functional Spec** — Waves 1–2 Spec **ACCEPTED** + **IMPLEMENTED ON MAIN**. Waves 3–5 **SPECIFIED / WAVE-GATED**. |

Wave 1 and Wave 2 engineering gates exited after Independent QA PASS (Rekik), merge, and DER. Canonical routes: `/restaurant/pms/guests` and `/restaurant/pms/guests/$guestId`. Catalogue key `guest-profile` (`partial`). Guest Services (`guest-services`) was not repurposed. Wave 2 CURRENT: Identity LIVE, Preferences card LIVE (Setup-owned; Preferences tab selects the card), controlled merge, consent. Production migration `0051_pms_guest_profile_wave2` remains **held** (Abel / PM); non-prod applied. Waves 3–5 wait for explicit ungating. Hotel UAT is still required for module COMPLETE.
...
| Guest Profile (`/restaurant/pms/guests`, `/restaurant/pms/guests/$guestId`) | [guests.md](./guests.md) — Waves 1–2 CURRENT on `main`; Waves 3–5 still gated; module **not** COMPLETE |
