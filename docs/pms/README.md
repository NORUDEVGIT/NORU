...
Official NORU Property Management System (PMS) docs. Some pages record features that have completed a coordination cycle (Advisor → Spec → Developer → QA → Docs). This tree also holds Functional Specs that stay **WAVE-GATED** until the named wave is accepted (Guest Profile Waves 1–4 are on `main`; Wave 5 is still gated).
...
| Guest Profile (`/restaurant/pms/guests`, `/restaurant/pms/guests/$guestId`) | [guests.md](./guests.md) — Waves 1–4 CURRENT on `main`; Wave 5 still gated; module **not** COMPLETE |
