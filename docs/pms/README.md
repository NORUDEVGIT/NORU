...
Official NORU Property Management System (PMS) docs. Some pages record features that have completed a coordination cycle (Advisor → Spec → Developer → QA → Docs). Guest Profile Waves 1–5 are **IMPLEMENTED ON MAIN**. The module is **not** COMPLETE — hotel UAT still required. Other Functional Specs stay **WAVE-GATED** until the named wave is accepted.
...
| Guest Profile (`/restaurant/pms/guests`, `/restaurant/pms/guests/$guestId`) | [guests.md](./guests.md) — Waves 1–5 **IMPLEMENTED ON MAIN**; module **not** COMPLETE |
