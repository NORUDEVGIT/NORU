...
| Migration 0057 | **On `main` from #111** (`0057_pms_individual_form_enrichment.sql`). **APPLY HELD** — **non-prod also NOT applied** (not only production); production Abel-gated. Surfaces that need GE2 columns / `guest_emergency_contacts` degrade to `INDIVIDUAL_ENRICHMENT_UNAVAILABLE` until apply. | Non-prod apply after Abel / PM YES, then production only with explicit approval. Do **not** claim applied from #111 / #112 / this recon. |
...
### 7.11 Migration 0057 hold (Gap-edit 2)
...
| Environment | Status |
|---|---|
| Non-prod `qcwptraosaudcbjasmul` | **NOT applied** — **APPLY HELD** (Abel / PM). Residual: non-prod is **also** unapplied, **not only** production. |
| Production | **NOT applied** — Abel / PM gate |

Until apply, Individual enrichment surfaces that need the new columns / `guest_emergency_contacts` degrade to `INDIVIDUAL_ENRICHMENT_UNAVAILABLE`. Do **not** treat PR [#111](https://github.com/NORUDEVGIT/NORU/pull/111) or [#112](https://github.com/NORUDEVGIT/NORU/pull/112) merge as schema apply. Do **not** overwrite Polish Wave 1 `0056`. Do **not** claim non-prod 0057 applied.
...
| Issue [#109](https://github.com/NORUDEVGIT/NORU/issues/109) | Gap-edit 2 Individual form enrichment + blacklist/restricted + Linking — **OPEN** (READY TO CLOSE). Advisor Outcome Review **recommends OPERATIONALLY ACCEPTED**. Rekik formal closure **pending YES**. PRs [#111](https://github.com/NORUDEVGIT/NORU/pull/111) + [#112](https://github.com/NORUDEVGIT/NORU/pull/112) Relates (did not Closes). Additive; does **not** reopen Waves 1–5 or rewrite #103. Migration `0057` **APPLY HELD** — non-prod **also NOT applied**. |
...
| [Functional Spec §9 — Gap-edit 2](./specs/guest-profile-module.md#9-gap-edit-2--individual-form-enrichment--blacklist--restricted) | Individual form enrichment + blacklist/restricted + Linking/Relationships — **IMPLEMENTED ON MAIN**. ENGINEERING STATUS **COMPLETE / MERGED**. IMPLEMENTATION STATUS **PASS**. HOLD **cleared** via [#112](https://github.com/NORUDEVGIT/NORU/pull/112). Advisor Outcome Review **recommends OPERATIONALLY ACCEPTED**; Rekik formal closure **pending YES**. **AC-GE2-1…34 PASS**. Additive; does **not** reopen Waves 1–5 or rewrite Gap-edit 1. Migration `0057` **APPLY HELD** (non-prod **also NOT applied**). |
...
| Backend / DB | Additive dual-lane `0057_pms_individual_form_enrichment.sql` **on `main` from #111**. **APPLY HELD** — **non-prod also NOT applied** (not only production); production **Abel-gated NOT applied**. |
...
- Receptionist vs owner/manager RLS inconsistency remains **PRESERVED**.
- Migration **`0057` APPLY HELD**. On `main` from #111. **Non-prod also NOT applied** (not only production). Production Abel-gated. Surfaces degrade honestly until apply. Do **not** claim 0057 applied on either environment.
