...
| **Gap-edit 2 Spec** | **ACCEPTED** + **IMPLEMENTED ON MAIN** — [Functional Spec §9](./specs/guest-profile-module.md#9-gap-edit-2--individual-form-enrichment--blacklist--restricted). ENGINEERING STATUS **COMPLETE / MERGED**. IMPLEMENTATION STATUS **PASS**. HOLD **cleared** via [#112](https://github.com/NORUDEVGIT/NORU/pull/112). PRs [#111](https://github.com/NORUDEVGIT/NORU/pull/111) + [#112](https://github.com/NORUDEVGIT/NORU/pull/112) MERGED. Spec [#110](https://github.com/NORUDEVGIT/NORU/pull/110) MERGED. AC-GE2-1…34 **PASS**. Independent QA **PASS** on #112. Issue [#109](https://github.com/NORUDEVGIT/NORU/issues/109) **OPEN** (READY TO CLOSE YES; Relates, not Closes). Does **not** reopen Waves 1–5 or rewrite Gap-edit 1. Migration `0057` **APPLY HELD** (not applied). Module COMPLETE remains **NO**. |
| **Implementation rule** | **Extend existing guest code — do NOT restart** |
| **Engineering assignment** | Waves 1–5 engineering complete / exited on `main`. Wave 5 **OPERATIONALLY ACCEPTED**. Gap-edit 2 **IMPLEMENTED ON MAIN** (#111 + #112); HOLD **cleared**. Programme **READY FOR HOTEL UAT**. Module COMPLETE remains **NO**. |
...
| Migration 0057 | **On `main` from #111** (`0057_pms_individual_form_enrichment.sql`). **APPLY HELD** — non-prod **NOT** applied; production Abel-gated. Surfaces that need GE2 columns / `guest_emergency_contacts` degrade to `INDIVIDUAL_ENRICHMENT_UNAVAILABLE` until apply. | Non-prod apply after Abel / PM YES, then production only with explicit approval. Do **not** claim applied from #111 / #112 / this recon. |
...
| Environment | Status |
|---|---|
| Non-prod `qcwptraosaudcbjasmul` | **NOT applied** — **APPLY HELD** (Abel / PM) |
| Production | **NOT applied** — Abel / PM gate |

Until apply, Individual enrichment surfaces that need the new columns / `guest_emergency_contacts` degrade to `INDIVIDUAL_ENRICHMENT_UNAVAILABLE`. Do **not** treat PR [#111](https://github.com/NORUDEVGIT/NORU/pull/111) or [#112](https://github.com/NORUDEVGIT/NORU/pull/112) merge as schema apply. Do **not** overwrite Polish Wave 1 `0056`.
...
| Record | Role |
|---|---|
| [Functional Spec — Guest Profile Module](./specs/guest-profile-module.md) | Master Spec: Wave 1–4 ACs + DER results; Wave 5 **IMPLEMENTED ON MAIN** (#101) — AC-W5-1…23 **PASS**; Gap-edit 2 **IMPLEMENTED ON MAIN** (#111 + #112) — AC-GE2-1…34 **PASS**; HOLD **cleared** |
...
| Issue [#109](https://github.com/NORUDEVGIT/NORU/issues/109) | Gap-edit 2 Individual form enrichment + blacklist/restricted + Linking — **OPEN** (READY TO CLOSE YES). PRs [#111](https://github.com/NORUDEVGIT/NORU/pull/111) + [#112](https://github.com/NORUDEVGIT/NORU/pull/112) Relates (did not Closes). Closure pending Rekik formal closure / Outcome Review. Additive; does **not** reopen Waves 1–5 or rewrite #103. Migration `0057` **APPLY HELD** (not applied). |
...
| [Functional Spec §9 — Gap-edit 2](./specs/guest-profile-module.md#9-gap-edit-2--individual-form-enrichment--blacklist--restricted) | Individual form enrichment + blacklist/restricted + Linking/Relationships — **IMPLEMENTED ON MAIN**. ENGINEERING STATUS **COMPLETE / MERGED**. IMPLEMENTATION STATUS **PASS**. HOLD **cleared** via [#112](https://github.com/NORUDEVGIT/NORU/pull/112). **AC-GE2-1…34 PASS**. Additive; does **not** reopen Waves 1–5 or rewrite Gap-edit 1. Migration `0057` **APPLY HELD**. |
...
- **Gap-edit 2** (Individual form enrichment + blacklist/restricted + Linking/Relationships) later landed as an **additive** follow-on — **IMPLEMENTED ON MAIN** (#111 + #112). HOLD **cleared**. **Not** a Wave 1–5 defect and **not** a reopen. Migration `0057` **APPLY HELD** (not applied). Does **not** rewrite Gap-edit 1. See §14.
...
| Backend / DB | Additive dual-lane `0057_pms_individual_form_enrichment.sql` **on `main` from #111**. **APPLY HELD** — non-prod **NOT** applied; production **Abel-gated NOT applied**. |
...
| Issue | [#109](https://github.com/NORUDEVGIT/NORU/issues/109) **OPEN** (READY TO CLOSE YES). Closure pending Rekik formal closure / Outcome Review. |
...
- Receptionist vs owner/manager RLS inconsistency remains **PRESERVED**.
- Migration **`0057` APPLY HELD**. On `main` from #111. Non-prod **NOT** applied. Production Abel-gated. Surfaces degrade honestly until apply. Do **not** claim 0057 applied.
...
- Issue [#109](https://github.com/NORUDEVGIT/NORU/issues/109) **OPEN** (READY TO CLOSE YES). Do **not** treat READY TO CLOSE as closed.
...
> **Wave 1 IMPLEMENTED ON MAIN** (#66 / #67). **Wave 2 IMPLEMENTED ON MAIN** (#72 / #76 / #79). **Wave 3 IMPLEMENTED ON MAIN** (#81 / #85 / #87 / #90). Wave 3 is OPERATIONALLY ACCEPTED / closed. Wave 4 **code LIVE on `main`** (#97). Wave 5 **IMPLEMENTED ON MAIN** (#101). Wave 5 is **OPERATIONALLY ACCEPTED** / engineering exited. Programme **READY FOR HOTEL UAT**. **Gap-edit 2 IMPLEMENTED ON MAIN** ([§9](./specs/guest-profile-module.md#9-gap-edit-2--individual-form-enrichment--blacklist--restricted) / [#111](https://github.com/NORUDEVGIT/NORU/pull/111) + [#112](https://github.com/NORUDEVGIT/NORU/pull/112)). ENGINEERING STATUS **COMPLETE / MERGED**. IMPLEMENTATION STATUS **PASS**. HOLD **cleared**. Issue [#109](https://github.com/NORUDEVGIT/NORU/issues/109) **OPEN** (READY TO CLOSE YES). Does not reopen Waves 1–5 or rewrite Gap-edit 1. Migration `0057` **APPLY HELD**. Module COMPLETE remains **NO** until hotel UAT PASS + docs recon merge + Advisor module close.
>
