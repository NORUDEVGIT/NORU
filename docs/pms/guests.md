...
| **Gap-edit 2 Spec** | **ACCEPTED** + **IMPLEMENTED ON MAIN** — [Functional Spec §9](./specs/guest-profile-module.md#9-gap-edit-2--individual-form-enrichment--blacklist--restricted). ENGINEERING STATUS **COMPLETE / MERGED**. IMPLEMENTATION STATUS **PASS**. HOLD **cleared** via [#112](https://github.com/NORUDEVGIT/NORU/pull/112). Advisor Outcome Review **recommends OPERATIONALLY ACCEPTED**. Rekik formal closure **pending YES**. Issue [#109](https://github.com/NORUDEVGIT/NORU/issues/109) **OPEN** (READY TO CLOSE; Relates, not Closes). Do **not** claim OPERATIONALLY ACCEPTED / closed until Rekik YES. Does **not** reopen Waves 1–5 or rewrite Gap-edit 1. Non-prod `0057` **APPLY PASS** on `qcwptraosaudcbjasmul` (version `20260914234317` / `pms_individual_form_enrichment`; 16/16 columns; `guest_emergency_contacts` YES). **Production 0057 NOT applied** (Abel-gated). Module COMPLETE remains **NO**. |
| **Implementation rule** | **Extend existing guest code — do NOT restart** |
| **Engineering assignment** | Waves 1–5 engineering complete / exited on `main`. Wave 5 **OPERATIONALLY ACCEPTED**. Gap-edit 2 **IMPLEMENTED ON MAIN** (#111 + #112); HOLD **cleared**. Advisor Outcome Review **recommends OPERATIONALLY ACCEPTED**; Rekik formal closure **pending YES** on [#109](https://github.com/NORUDEVGIT/NORU/issues/109). Programme **READY FOR HOTEL UAT**. Module COMPLETE remains **NO**. |
| **Classification** | Functional Spec / programme — Waves 1–5 CURRENT on `main` for delivered cards; Gap-edit 2 Individual enrichment CURRENT on `main`; programme **READY FOR HOTEL UAT**; module **not** COMPLETE |
...
Engineering works **wave-by-wave**. A later wave does not start until the prior wave has exited (docs + product exit criteria). Full testable Wave 1–5 ACs (Wave 5 = **IMPLEMENTED ON MAIN**; IMPLEMENTATION STATUS **PASS**) are in the [Functional Spec](./specs/guest-profile-module.md).
...
Grounded in `main` at documentation time (`d6058d5`, after #111 Individual enrichment + #112 staged Create + #110 Spec HOLD amend). **Code wins.** Detail: [Functional Spec — CURRENT behaviour](./specs/guest-profile-module.md#2-current-behaviour-code-wins). Wave 5 CURRENT: [Spec §7.2](./specs/guest-profile-module.md#72-current-behaviour-wave-5-surfaces--code-wins). Gap-edit 2 CURRENT: [Spec §9.2](./specs/guest-profile-module.md#92-current-behaviour-code-wins). Wave 4 **code is LIVE**; issue [#95](https://github.com/NORUDEVGIT/NORU/issues/95) **CLOSED** completed. Wave 5 **IMPLEMENTED ON MAIN** (#101); issue [#98](https://github.com/NORUDEVGIT/NORU/issues/98) **CLOSED** completed. Gap-edit 2 **IMPLEMENTED ON MAIN** (#111 + #112). Advisor Outcome Review **recommends OPERATIONALLY ACCEPTED**. Rekik formal closure **pending YES**. Issue [#109](https://github.com/NORUDEVGIT/NORU/issues/109) **OPEN** (READY TO CLOSE).
...
| Masters / relationships | **LIVE in code** (#97): Guest-owned Company / Group / TA (`guest_account_masters`) + `guest_account_links`. Wave 4 docs recon on `main` (#99). FO `company_name` / `group_name` remain stay **labels** unless consumed as master IDs. SET3 `companyRelationshipEnabled` remains a Setup **flag**. Sales & Events is a planned placeholder. Masters export / anonymise contact PII (Wave 5). Individual **Linking staged on Create + after-save LIVE** (#112): search + pick Company / Group / TA + role before submit; one Create writes Wave 4 roles; list + unlink; Relationships card stays in sync. Partial-failure retry — never silent full success. | Same Guest-owned registers. Group account ≠ S&E blocks. Bill-to is an association — not folio routing while `transfersSupported: false`. Relationships card remains. No guest↔guest family. After-save-only on create **fails Spec** and is **not** CURRENT. |
...
This overview plus the Functional Spec remain the programme record. Waves 1–5 are implemented on `main`. Wave 5 is **OPERATIONALLY ACCEPTED** / engineering exited. Programme is **READY FOR HOTEL UAT**. Issue [#98](https://github.com/NORUDEVGIT/NORU/issues/98) **CLOSED** completed. Module COMPLETE remains **NO** until hotel UAT PASS + this docs recon merge + Advisor module close.
...
| Record | Role |
|---|---|
| [Functional Spec — Guest Profile Module](./specs/guest-profile-module.md) | Master Spec: Wave 1–4 ACs + DER results; Wave 5 **IMPLEMENTED ON MAIN** (#101) — AC-W5-1…23 **PASS**; Gap-edit 2 **IMPLEMENTED ON MAIN** (#111 + #112) — AC-GE2-1…34 **PASS**; HOLD **cleared**; Advisor Outcome Review **recommends OPERATIONALLY ACCEPTED**; Rekik formal closure **pending YES** |
...
| [decisions/README.md](./decisions/README.md) | Short note: Waves 1–5 implemented on `main`; module **not** COMPLETE. |
...
| Issue [#109](https://github.com/NORUDEVGIT/NORU/issues/109) | Gap-edit 2 Individual form enrichment + blacklist/restricted + Linking — **OPEN** (READY TO CLOSE). Advisor Outcome Review **recommends OPERATIONALLY ACCEPTED**. Rekik formal closure **pending YES**. PRs [#111](https://github.com/NORUDEVGIT/NORU/pull/111) + [#112](https://github.com/NORUDEVGIT/NORU/pull/112) Relates (did not Closes). Additive; does **not** reopen Waves 1–5 or rewrite #103. Non-prod `0057` **APPLY PASS** (`qcwptraosaudcbjasmul` / `20260914234317`). **Production 0057 NOT applied** (Abel-gated). |
...
| [Functional Spec §9 — Gap-edit 2](./specs/guest-profile-module.md#9-gap-edit-2--individual-form-enrichment--blacklist--restricted) | Individual form enrichment + blacklist/restricted + Linking/Relationships — **IMPLEMENTED ON MAIN**. ENGINEERING STATUS **COMPLETE / MERGED**. IMPLEMENTATION STATUS **PASS**. HOLD **cleared** via [#112](https://github.com/NORUDEVGIT/NORU/pull/112). Advisor Outcome Review **recommends OPERATIONALLY ACCEPTED**; Rekik formal closure **pending YES**. **AC-GE2-1…34 PASS**. Additive; does **not** reopen Waves 1–5 or rewrite Gap-edit 1. Non-prod `0057` **APPLY PASS** (`qcwptraosaudcbjasmul` / `20260914234317`). **Production 0057 NOT applied** (Abel-gated). |
...
- **Gap-edit 2** (Individual form enrichment + blacklist/restricted + Linking/Relationships) later landed as an **additive** follow-on — **IMPLEMENTED ON MAIN** (#111 + #112). HOLD **cleared**. Advisor Outcome Review **recommends OPERATIONALLY ACCEPTED**; Rekik formal closure **pending YES**. **Not** a Wave 1–5 defect and **not** a reopen. Non-prod `0057` **APPLY PASS** (`qcwptraosaudcbjasmul` / `20260914234317`). **Production 0057 NOT applied** (Abel-gated). Does **not** rewrite Gap-edit 1. See §14.

---

## 14. QA / implementation status (Gap-edit 2)
...
| Advisor Outcome Review | **Recommends OPERATIONALLY ACCEPTED** |
| Rekik formal closure | **Pending YES** — do **not** claim OPERATIONALLY ACCEPTED / closed |
...
| Issue | [#109](https://github.com/NORUDEVGIT/NORU/issues/109) **OPEN** (READY TO CLOSE). Advisor Outcome Review **recommends OPERATIONALLY ACCEPTED**. Rekik formal closure **pending YES**. |
...
- Advisor Outcome Review **recommends OPERATIONALLY ACCEPTED**. Rekik formal closure **pending YES**. Issue [#109](https://github.com/NORUDEVGIT/NORU/issues/109) **OPEN** (READY TO CLOSE). Do **not** treat READY TO CLOSE or the Advisor recommendation as closed / OPERATIONALLY ACCEPTED.
...
- This recon does **not** reopen Waves 1–5 or rewrite Gap-edit 1. Gap-edit 1 docs recon [#108](https://github.com/NORUDEVGIT/NORU/pull/108) may still be open — this section is Gap-edit 2 only.

---

## Closing

> **Wave 1 IMPLEMENTED ON MAIN** (#66 / #67). **Wave 2 IMPLEMENTED ON MAIN** (#72 / #76 / #79). **Wave 3 IMPLEMENTED ON MAIN** (#81 / #85 / #87 / #90). Wave 3 is OPERATIONALLY ACCEPTED / closed. Wave 4 **code LIVE on `main`** (#97). Wave 5 **IMPLEMENTED ON MAIN** (#101). Wave 5 is **OPERATIONALLY ACCEPTED** / engineering exited. Programme **READY FOR HOTEL UAT**. **Gap-edit 2 IMPLEMENTED ON MAIN** ([§9](./specs/guest-profile-module.md#9-gap-edit-2--individual-form-enrichment--blacklist--restricted) / [#111](https://github.com/NORUDEVGIT/NORU/pull/111) + [#112](https://github.com/NORUDEVGIT/NORU/pull/112)). ENGINEERING STATUS **COMPLETE / MERGED**. IMPLEMENTATION STATUS **PASS**. HOLD **cleared**. Advisor Outcome Review **recommends OPERATIONALLY ACCEPTED**. Rekik formal closure **pending YES**. Issue [#109](https://github.com/NORUDEVGIT/NORU/issues/109) **OPEN** (READY TO CLOSE). Does not reopen Waves 1–5 or rewrite Gap-edit 1. Non-prod `0057` **APPLY PASS** (`qcwptraosaudcbjasmul` / `20260914234317`). **Production 0057 NOT applied** (Abel-gated). Module COMPLETE remains **NO** until hotel UAT PASS + docs recon merge + Advisor module close.
>
