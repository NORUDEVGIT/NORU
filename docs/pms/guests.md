...
| **Wave 4 Spec** | **Code LIVE on `main`** — PR [#97](https://github.com/NORUDEVGIT/NORU/pull/97) MERGED. Issue [#95](https://github.com/NORUDEVGIT/NORU/issues/95) **CLOSED** completed. This Wave 5 catch-up does **not** reopen Wave 4. |
| **Wave 5 Spec** | **ACCEPTED for Engineering** (Rekik 2026-09-14) — Eng-ready catch-up. Gate **OPENED**; plan **APPROVED**. ENGINEERING **IMPLEMENTATION IN PROGRESS**. Issue [#98](https://github.com/NORUDEVGIT/NORU/issues/98) OPEN. Branch `feature/98-guest-profile-wave-5`. **Not** implemented (no PASS / LIVE / COMPLETE). |
| **Gap-edit 1 Spec** | **ACCEPTED for Engineering** (Rekik 2026-09-14) — Eng-ready catch-up. Gate **OPENED**; tech plan **APPROVED**. ENGINEERING **IMPLEMENTATION IN PROGRESS**. Issue [#103](https://github.com/NORUDEVGIT/NORU/issues/103) OPEN. Branch `feature/103-company-registration-enrichment`. **Not** implemented (no PASS / LIVE / COMPLETE). Additive on Wave 4 `guest_account_masters` / `guest_account_links`. Does **not** reopen Waves 1–5. Migration `0055` Abel/PM gated. |
...
| **Classification** | Functional Spec / programme — Waves 1–3 CURRENT on `main`; Wave 4 **code LIVE** (#97); Wave 5 Spec **ACCEPTED for Engineering** (implementation in progress); Gap-edit 1 Spec **ACCEPTED for Engineering** (implementation in progress; **not** implemented); **not** CURRENT that every card is LIVE |
...
>
> **Wave 4 code LIVE on `main`** (Rekik 2026-09-14). PR [#97](https://github.com/NORUDEVGIT/NORU/pull/97) MERGED. Issue [#95](https://github.com/NORUDEVGIT/NORU/issues/95) **CLOSED** completed. Do **not** claim Wave 4 unimplemented. This Wave 5 catch-up does **not** reopen Wave 4.
>
> **Wave 5 Spec ACCEPTED for Engineering** (Rekik 2026-09-14). Gate **OPENED**. Technical Implementation Plan **APPROVED** via Advisor. ENGINEERING STATUS **IMPLEMENTATION IN PROGRESS**. Issue [#98](https://github.com/NORUDEVGIT/NORU/issues/98) OPEN. Coding AUTHORIZED on `feature/98-guest-profile-wave-5`. Wave 5 is **not** implemented.
>
> **Gap-edit 1 Spec ACCEPTED for Engineering** (Rekik 2026-09-14 via Hospitality Product Advisor). Eng-ready catch-up. Gate **OPENED**. Technical Implementation Plan **APPROVED** 2026-09-14. ENGINEERING **IMPLEMENTATION IN PROGRESS**. Issue [#103](https://github.com/NORUDEVGIT/NORU/issues/103) OPEN. Coding AUTHORIZED on `feature/103-company-registration-enrichment`. Gap-edit 1 is **not** implemented (no PASS / LIVE / COMPLETE). Additive Company master UX/schema on Wave 4 masters. Waves 1–5 remain OPERATIONALLY ACCEPTED / eng exited **where documented**. This gap-edit does **not** reopen those waves. Wave 5 docs recon may still be in flight ([#102](https://github.com/NORUDEVGIT/NORU/pull/102)). Migration `0055` Abel/PM gated. Module COMPLETE still needs hotel UAT.
>
> Create once → use everywhere → enrich. The module is **COMPLETE** only after Waves 1–5 **and** hotel UAT. Do **not** treat Wave 1–4 merge as module COMPLETE. Do **not** treat this Wave 5 Spec catch-up as Wave 5 PASS / LIVE / COMPLETE. Do **not** treat Gap-edit 1 as implemented / PASS / LIVE / COMPLETE.
>
...
Company / Group / TA **masters** are Wave 4 and **LIVE in code** on `main` (#97). Formal Wave 4 docs recon may still be in flight. Waves 1–3 must not imply those masters were LIVE at Wave 3 exit.
...
| 9 | **Notes / Comms / Activity** | 5 (product hub); notes + profile history exist today | Notes ≠ comms product; no fake send |
...
Engineering works **wave-by-wave**. A later wave does not start until the prior wave has exited (docs + product exit criteria). Full testable Wave 1–5 ACs (Wave 5 = **accepted engineering contract**; implementation **in progress**) are in the [Functional Spec](./specs/guest-profile-module.md).
...
| **5** | Comms / Activity + Privacy finish (export / anonymise / unmerge) for individuals and masters as appropriate | **IMPLEMENTATION IN PROGRESS** — Spec **ACCEPTED for Engineering**. Gate **OPENED**; Rekik plan **APPROVED** 2026-09-14. Issue [#98](https://github.com/NORUDEVGIT/NORU/issues/98) OPEN. Branch `feature/98-guest-profile-wave-5`. **Not** implemented. | Full hub + privacy honesty + hotel UAT-ready |
| **Gap-edit 1** | Company registration enrichment (sectioned Company form + multi-guest link) | **IMPLEMENTATION IN PROGRESS** — Spec **ACCEPTED for Engineering**. Gate **OPENED**; Rekik tech plan **APPROVED** 2026-09-14. Issue [#103](https://github.com/NORUDEVGIT/NORU/issues/103) OPEN. Branch `feature/103-company-registration-enrichment`. **Not** implemented. Additive on Wave 4 Company masters. Does **not** reopen Waves 1–5. Migration `0055` Abel/PM gated. | Sectioned Company form + Directory multi-link via existing `guest_account_links` |
...
Grounded in `main` at documentation time (`37af44d`, after #97 Wave 4 code merge). **Code wins.** Detail: [Functional Spec — CURRENT behaviour](./specs/guest-profile-module.md#2-current-behaviour-code-wins). Wave 5 CURRENT vs EXPECTED: [Spec §7.2–§7.3](./specs/guest-profile-module.md#72-current-behaviour-wave-5-surfaces--code-wins). Wave 4 **code is LIVE**; issue [#95](https://github.com/NORUDEVGIT/NORU/issues/95) **CLOSED** completed — do not treat stale Wave 4 planning CURRENT tables as a claim that masters are unimplemented.
...
| Duplicates / merge | Wave 1 warn remains: Open existing / Create anyway — **no auto-merge**. Controlled `mergeGuests` behind explicit confirm. Soft-retire (`inactive` + `merged_into_guest_id`); reassign `hotel_reservations.guest_id`; move documents; history `merged_from` / `merged_into`. Dialog: Wave 2 cannot unmerge. | Unmerge is Wave 5 — never silent. |
| Consent | Data-processing and marketing: `granted` / `refused` / `not_asked` + recorded at / by. Visible on Information. SET3 `consentDefaults` are guidance only when `not_asked`. History `consent_updated`. | Wave 5 privacy suite (export / anonymise / unmerge / audit) builds on this record. Consent stays visible and editable under policy. |
| Masters / relationships | **LIVE in code** (#97): Guest-owned Company / Group / TA (`guest_account_masters`) + `guest_account_links`. Formal Wave 4 docs recon may still be in flight. FO `company_name` / `group_name` remain stay **labels** unless consumed as master IDs. SET3 `companyRelationshipEnabled` remains a Setup **flag**. Sales & Events is a planned placeholder. Company form is **thin** (required name; optional code / phone / email / address / city / country / notes). Master-side link is single-select on Relationships. | Same Guest-owned registers. Group account ≠ S&E blocks. Bill-to is an association — not folio routing while `transfersSupported: false`. **Gap-edit 1** enriches **Company** only: sectioned form + multi-select Directory link (`employer` default / `bill_to`) on existing `guest_account_links`. Group/TA forms stay thin. |
| Loyalty | **LIVE in code** (#97): real-derived stay / night / stored amounts or honest empty. **No invented points.** VIP remains a staff flag. SET3 `pms_guest_vip_levels` is a Setup catalogue, not points. Formal Wave 4 docs recon may still be in flight. | Same honesty. **No invented points.** |
...
This overview plus the Functional Spec remain the programme record. Waves 1–3 are implemented on `main`. Wave 4 **code is LIVE on `main`** (#97); issue [#95](https://github.com/NORUDEVGIT/NORU/issues/95) **CLOSED** completed. Wave 5 Spec is **ACCEPTED for Engineering** (gate OPENED; plan APPROVED; implementation **IN PROGRESS**; **not** implemented). Gap-edit 1 Spec is **ACCEPTED for Engineering** (gate OPENED; plan APPROVED; implementation **IN PROGRESS** on [#103](https://github.com/NORUDEVGIT/NORU/issues/103); **not** implemented). Waves 1–5 **not** reopened. Migration `0055` Abel/PM gated. The module is **not** COMPLETE until Wave 5 and hotel UAT also pass.
...
| Record | Role |
|---|---|
| [Functional Spec — Guest Profile Module](./specs/guest-profile-module.md) | Master Spec: Wave 1–3 ACs + DER results; Wave 4 code LIVE (#97); Wave 5 **ACCEPTED for Engineering** (AC-W5-1…23); Gap-edit 1 **ACCEPTED for Engineering** (§8, AC-GE1-1…24; implementation **IN PROGRESS** on #103; **not** implemented) |
...
| [decisions/README.md](./decisions/README.md) | Short note: Waves 1–3 implemented; Wave 4 code LIVE; Wave 5 ACCEPTED for Engineering / implementation in progress; Gap-edit 1 ACCEPTED for Engineering / implementation in progress on #103. |
| Issue [#98](https://github.com/NORUDEVGIT/NORU/issues/98) | Wave 5 tracking issue — **OPEN**. Rekik plan **APPROVED**. Coding AUTHORIZED on `feature/98-guest-profile-wave-5`. **Not** implemented. |
...
- Wave 4 **code is LIVE on `main`** (#97). Issue [#95](https://github.com/NORUDEVGIT/NORU/issues/95) **CLOSED** completed. Wave 5 Spec is **ACCEPTED for Engineering** (implementation **IN PROGRESS**; see §4 / Spec §7). Hotel UAT is still required for **module COMPLETE**.
...
> **Wave 1 IMPLEMENTED ON MAIN** (#66 / #67). **Wave 2 IMPLEMENTED ON MAIN** (#72 / #76 / #79). **Wave 3 IMPLEMENTED ON MAIN** (#81 / #85 / #87 / #90). Wave 3 is OPERATIONALLY ACCEPTED / closed. Wave 4 **code LIVE on `main`** (#97). Wave 5 Spec **ACCEPTED for Engineering** (implementation **IN PROGRESS**; **not** implemented). Gap-edit 1 Spec **ACCEPTED for Engineering** (implementation **IN PROGRESS** on #103; **not** implemented; additive; Waves 1–5 not reopened). Migration `0055` Abel/PM gated. Module COMPLETE only after Waves 1–5 + hotel UAT.
>
