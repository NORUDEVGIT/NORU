...
| **Gap-edit 2 Spec** | **ACCEPTED for Engineering** — [Functional Spec §9](./specs/guest-profile-module.md#9-gap-edit-2--individual-form-enrichment--blacklist--restricted). ENGINEERING **IMPLEMENTATION IN PROGRESS** on [#109](https://github.com/NORUDEVGIT/NORU/issues/109) / `feature/109-individual-form-enrichment`. Plan **APPROVED** with Linking (**AC-GE2-9…13**). **Not** implemented / PASS / LIVE / COMPLETE. Does **not** reopen Waves 1–5 or Gap-edit 1. Migration `0056` Abel/PM gated. Module COMPLETE remains **NO**. |
...
>
> **Gap-edit 2 Spec ACCEPTED for Engineering** (Rekik APPROVED 2026-09-15 via Hospitality Product Advisor). CONDITIONAL PLAN APPROVAL **satisfied** (Linking REQUIRED already on the amended TIP). ENGINEERING **IMPLEMENTATION IN PROGRESS** on [#109](https://github.com/NORUDEVGIT/NORU/issues/109) / `feature/109-individual-form-enrichment`. Plan **APPROVED** with Linking. Contract: [Functional Spec §9](./specs/guest-profile-module.md#9-gap-edit-2--individual-form-enrichment--blacklist--restricted) (Linking **AC-GE2-9…13**). **Not** implemented / PASS / LIVE / COMPLETE. Does **not** reopen Waves 1–5. Does **not** reopen or rewrite Gap-edit 1 (§8 / #103 / #105). Migration **`0056`** Abel/PM gated (APPLY HELD). Module COMPLETE remains **NO**.
>
...
| Identity | Identity & Documents **card LIVE**: upload / list (signed URLs), staff verify / reject with actor + time. Ordinary Directory / Information **mask** the ID number (last four + Reveal). Copy is staff confirmation only — never government KYC. Storage: `guest_documents` + `property-images` at `{restaurantId}/guests/{guestId}/{uuid}.{ext}`. Form / Information Identity are **text-only** (card-discovered upload). | Same Wave 2 store. Gap-edit 2 **EXPECTED**: collocate upload on form + Information Identity (reuse `guest_documents` — no second store). |
...
| [Functional Spec §9 — Gap-edit 2](./specs/guest-profile-module.md#9-gap-edit-2--individual-form-enrichment--blacklist--restricted) | Individual form enrichment + blacklist/restricted + Linking/Relationships — **ACCEPTED for Engineering**. ENGINEERING **IN PROGRESS** on [#109](https://github.com/NORUDEVGIT/NORU/issues/109). Plan **APPROVED** with Linking. **AC-GE2-9…13** = Scope C. Additive; **not** implemented / PASS / LIVE / COMPLETE; does **not** reopen Waves 1–5 or rewrite Gap-edit 1. Migration `0056` Abel/PM gated. |
| Issue [#109](https://github.com/NORUDEVGIT/NORU/issues/109) | Gap-edit 2 tracking issue — **OPEN**. CONDITIONAL PLAN APPROVAL satisfied. Coding **AUTHORIZED** on `feature/109-individual-form-enrichment`. **Not** implemented. |
...
- **Gap-edit 2** (Individual form enrichment + blacklist/restricted + Linking/Relationships) is an **additive** follow-on — Spec **ACCEPTED for Engineering**; ENGINEERING **IN PROGRESS** on [§9](./specs/guest-profile-module.md#9-gap-edit-2--individual-form-enrichment--blacklist--restricted) / [#109](https://github.com/NORUDEVGIT/NORU/issues/109). Plan **APPROVED** with Linking (**AC-GE2-9…13**). **Not** a Wave 1–5 defect and **not** a reopen. **Not** implemented / PASS / LIVE / COMPLETE. Migration `0056` Abel/PM gated. Does **not** rewrite Gap-edit 1.

---

## Closing

> **Wave 1 IMPLEMENTED ON MAIN** (#66 / #67). **Wave 2 IMPLEMENTED ON MAIN** (#72 / #76 / #79). **Wave 3 IMPLEMENTED ON MAIN** (#81 / #85 / #87 / #90). Wave 3 is OPERATIONALLY ACCEPTED / closed. Wave 4 **code LIVE on `main`** (#97). Wave 5 **IMPLEMENTED ON MAIN** (#101). Wave 5 is **OPERATIONALLY ACCEPTED** / engineering exited. Programme **READY FOR HOTEL UAT**. **Gap-edit 2 Spec ACCEPTED for Engineering** — ENGINEERING **IN PROGRESS** ([§9](./specs/guest-profile-module.md#9-gap-edit-2--individual-form-enrichment--blacklist--restricted) / [#109](https://github.com/NORUDEVGIT/NORU/issues/109)); plan APPROVED with Linking; **not** implemented. Does not reopen Waves 1–5 or Gap-edit 1. Module COMPLETE remains **NO** until hotel UAT PASS + docs recon merge + Advisor module close.
>
