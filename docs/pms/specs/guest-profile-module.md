...
| **Gap-edit 1 Spec** | **ACCEPTED** + **IMPLEMENTED ON MAIN**. ENGINEERING STATUS **COMPLETE / MERGED**. IMPLEMENTATION STATUS **PASS**. PR [#105](https://github.com/NORUDEVGIT/NORU/pull/105) MERGED 2026-09-14T18:55:43Z (Relates #103 — did not Closes). **OPERATIONALLY ACCEPTED** / closed (Rekik formal closure YES / Advisor Outcome Review). Issue [#103](https://github.com/NORUDEVGIT/NORU/issues/103) **CLOSED** completed 2026-09-14T22:43:40Z. Additive Company enrichment — does **not** reopen Waves 1–5. Module COMPLETE remains **NO**. |
| **Gap-edit 2 Spec** | **ACCEPTED** + **IMPLEMENTED ON MAIN**. ENGINEERING STATUS **COMPLETE / MERGED**. IMPLEMENTATION STATUS **PASS**. HOLD **cleared** via [#112](https://github.com/NORUDEVGIT/NORU/pull/112). **OPERATIONALLY ACCEPTED** / closed (Rekik formal closure YES / Advisor Outcome Review). Issue [#109](https://github.com/NORUDEVGIT/NORU/issues/109) **CLOSED** completed 2026-09-14T23:46:37Z (Rekik formal closure YES / Advisor Outcome Review). Does **not** reopen Waves 1–5 or rewrite Gap-edit 1 (§8). Non-prod `0057` **APPLY PASS** on `qcwptraosaudcbjasmul` (version `20260914234317` / `pms_individual_form_enrichment`; 16/16 columns; `guest_emergency_contacts` YES). **Production 0057 NOT applied** (Abel-gated). Module COMPLETE remains **NO**. |
| **Implementation rule** | **Extend existing guest code — do NOT restart** |
| **Engineering assignment** | Waves 1–5 engineering complete / exited on `main`. Wave 5 **OPERATIONALLY ACCEPTED**. Gap-edit 1 **IMPLEMENTED ON MAIN** (#105) and **OPERATIONALLY ACCEPTED** / closed. Gap-edit 2 **IMPLEMENTED ON MAIN** (#111 + #112); HOLD **cleared**. **OPERATIONALLY ACCEPTED** / closed (Rekik formal closure YES). Issue [#109](https://github.com/NORUDEVGIT/NORU/issues/109) **CLOSED** completed 2026-09-14T23:46:37Z. Programme **READY FOR HOTEL UAT**. Module COMPLETE remains **NO**. |
| **Product requirement** | Rekik 2026-09-14 — Waves 1–5 accepted and implemented; Gap-edit 1 implemented on `main`; Gap-edit 2 implemented on `main`; programme **READY FOR HOTEL UAT**; module COMPLETE remains **NO** |
...
>
> **Gap-edit 1 IMPLEMENTED ON MAIN** (Rekik 2026-09-14 / DER after #105). ENGINEERING STATUS **COMPLETE / MERGED**. IMPLEMENTATION STATUS **PASS**. PR [#105](https://github.com/NORUDEVGIT/NORU/pull/105) MERGED 2026-09-14T18:55:43Z (Relates #103 — did not Closes). Independent QA **PASS**; Developer `tsc` + locks **107/107 PASS**. AC-GE1-1…24 **PASS**. Non-prod 0055 **APPLY PASS** on `qcwptraosaudcbjasmul` (version `20260914221747`, 15 enrichment columns). Production 0055 **NOT applied** (Abel / PM gated). **OPERATIONALLY ACCEPTED** / closed (Rekik formal closure YES / Advisor Outcome Review). Issue [#103](https://github.com/NORUDEVGIT/NORU/issues/103) **CLOSED** completed 2026-09-14T22:43:40Z. Additive Company enrichment on Wave 4 `guest_account_masters` / `guest_account_links`. Waves 1–5 remain OPERATIONALLY ACCEPTED / eng exited **where documented**. This recon does **not** reopen those waves. Wave 5 docs recon [#102](https://github.com/NORUDEVGIT/NORU/pull/102) is **MERGED**. Do **not** claim the **module** COMPLETE (hotel UAT still required).
>
> **Gap-edit 2 IMPLEMENTED ON MAIN** (Rekik / DER after #111 + #112). ENGINEERING STATUS **COMPLETE / MERGED**. IMPLEMENTATION STATUS **PASS**. HOLD **cleared** via [#112](https://github.com/NORUDEVGIT/NORU/pull/112) (staged Create). PR [#111](https://github.com/NORUDEVGIT/NORU/pull/111) MERGED early (enrichment without staged Create) · PR [#112](https://github.com/NORUDEVGIT/NORU/pull/112) MERGED 2026-09-14T23:30:45Z · Spec [#110](https://github.com/NORUDEVGIT/NORU/pull/110) MERGED (HOLD amend AC-GE2-9…14). Independent QA **PASS** on #112. AC-GE2-1…34 **PASS**. Locks **142/142 PASS**. Developer browser **NOT RUN**. **OPERATIONALLY ACCEPTED** / closed (Rekik formal closure YES / Advisor Outcome Review). Issue [#109](https://github.com/NORUDEVGIT/NORU/issues/109) **CLOSED** completed 2026-09-14T23:46:37Z (Rekik formal closure YES / Advisor Outcome Review). Staged Create LIVE: create → `guest_documents` → `guest_account_links`; partial-failure retry; never silent full success. After-save-only on create **fails Spec**. See **§9**. This recon does **not** reopen Waves 1–5 as incomplete. It does **not** reopen or rewrite Gap-edit 1 (§8 / #103 / #105). Non-prod `0057` **APPLY PASS** on `qcwptraosaudcbjasmul` (version `20260914234317` / `pms_individual_form_enrichment`; 16/16 enrichment+restriction columns; `guest_emergency_contacts` YES). **Production 0057 NOT applied** (Abel-gated). Production surfaces degrade honestly until apply. The module is **not** COMPLETE (hotel UAT still required).
>
...
This document is the master Functional Spec for the Guest Profile Module. Wave 1 ACs remain the accepted contract. Wave 2 ACs in §4 remain the accepted contract (now implemented). Wave 3 ACs in §5 remain the accepted contract (now implemented). Wave 4 ACs in §6 remain the accepted contract (now implemented; AC-W4-5 PARTIAL residual). Wave 5 ACs in §7 are the accepted contract and are **IMPLEMENTED ON MAIN** (#101). Wave 5 is **OPERATIONALLY ACCEPTED** / engineering exited. Issue [#98](https://github.com/NORUDEVGIT/NORU/issues/98) **CLOSED** completed. Gap-edit 1 ACs in §8 are the accepted contract and are **IMPLEMENTED ON MAIN** (#105). **OPERATIONALLY ACCEPTED** / closed (Rekik formal closure YES / Advisor Outcome Review). Issue [#103](https://github.com/NORUDEVGIT/NORU/issues/103) **CLOSED** completed 2026-09-14T22:43:40Z. Gap-edit 2 ACs in **§9** are **IMPLEMENTED ON MAIN** (#111 + #112). ENGINEERING STATUS **COMPLETE / MERGED**. IMPLEMENTATION STATUS **PASS**. HOLD **cleared**. **OPERATIONALLY ACCEPTED** / closed (Rekik formal closure YES / Advisor Outcome Review). Issue [#109](https://github.com/NORUDEVGIT/NORU/issues/109) is **CLOSED** completed 2026-09-14T23:46:37Z (Rekik formal closure YES / Advisor Outcome Review). Programme is **READY FOR HOTEL UAT**. Module COMPLETE remains **NO**.
...
| Master Directory / Information | Company / Group / TA create / edit / list / search. **Company form LIVE** as a sectioned modal (Basic open by default) after [#105](https://github.com/NORUDEVGIT/NORU/pull/105): type enum, tax, contact alts, address line2/region/postal, commercial refs, notes, default TA FK. Company detail multi-guest link / list / unlink via `guest_account_links`. Individual Relationships remains. Group / TA forms stay thin. Group copy says **account master**, not S&E block. Masters export / anonymise contact PII. |
...
| Production schema 0055 | Non-prod `qcwptraosaudcbjasmul` applied (`20260914221747` / `pms_company_registration_enrichment`, 15 enrichment columns). **Production NOT applied** (Abel / PM gated). Company enrichment surfaces degrade to `COMPANY_ENRICHMENT_UNAVAILABLE` until apply. |
| Production schema 0057 | Non-prod `qcwptraosaudcbjasmul` **APPLY PASS** (`20260914234317` / `pms_individual_form_enrichment`; 16/16 columns; `guest_emergency_contacts` YES). **Production NOT applied** (Abel-gated). Production GE2 surfaces degrade to `INDIVIDUAL_ENRICHMENT_UNAVAILABLE` until apply. |

| Preference id mapping | Stored as `id:` / `other:` prefixes in text columns — honesty item, not a Setup FK. |
| Sectioned Individual form + emergency / employment / alt contact | **LIVE on `main`** (#111 + #112) — sectioned Individual modal; Employment / Emergency / Notes. See **§9**. Not a Wave 1–2 defect. |
| Blacklist / restricted product flag | **LIVE on `main`** (#111) — restricted / blacklisted badges + history + stay-path **warn**. See **§9**. |
...
- No Company rate engine. Negotiated rate reference is name/code text only.
- No Position / Department on the Company form. Group / TA forms stay thin.
...
**Minimum identity for search:** a display **name** (required) so staff can find the master they just created. Optional contact / notes on `guest_account_masters`: code, phone, email, address, city, country, notes. Gap-edit 1 later **enriched Company only** (sectioned form + multi-guest link) — see §8. Wave 4 thin Group / TA register **remains accepted**. This does **not** reopen Wave 4.
...
| **PMS AREA** | Guests |
| **SPEC STATUS** | **ACCEPTED** + **IMPLEMENTED ON MAIN** (Rekik 2026-09-14) |
| **ENGINEERING STATUS** | **COMPLETE / MERGED** — PR [#105](https://github.com/NORUDEVGIT/NORU/pull/105) MERGED 2026-09-14T18:55:43Z (Relates #103 — did not Closes). IMPLEMENTATION STATUS **PASS**. DESIGN COMPLETION **COMPLETE** (Gap-edit 1). **OPERATIONALLY ACCEPTED** / closed (Rekik formal closure YES / Advisor Outcome Review). Issue [#103](https://github.com/NORUDEVGIT/NORU/issues/103) **CLOSED** completed 2026-09-14T22:43:40Z. Production `0055` Abel/PM gated. Module COMPLETE remains **NO**. |
...
| **Depends on** | Wave 4 **code LIVE on `main`** (#97) — `guest_account_masters` / `guest_account_links` / `guest_account_history`. This gap-edit **extends** those tables. It does **not** reopen Wave 4. Waves 1–5 remain OPERATIONALLY ACCEPTED / eng exited **where documented**. Wave 5 docs recon [#102](https://github.com/NORUDEVGIT/NORU/pull/102) is **MERGED**. Spec [#104](https://github.com/NORUDEVGIT/NORU/pull/104) MERGED. |
| **REQUIREMENTS** | **Locked** from the product requirement (Rekik APPROVED 2026-09-14) |

> **Gap-edit 1 IMPLEMENTED ON MAIN** (Rekik 2026-09-14 / DER after #105). ENGINEERING STATUS **COMPLETE / MERGED**. IMPLEMENTATION STATUS **PASS**. **OPERATIONALLY ACCEPTED** / closed (Rekik formal closure YES / Advisor Outcome Review). PR [#105](https://github.com/NORUDEVGIT/NORU/pull/105) MERGED 2026-09-14T18:55:43Z. Spec [#104](https://github.com/NORUDEVGIT/NORU/pull/104) MERGED. Issue [#103](https://github.com/NORUDEVGIT/NORU/issues/103) **CLOSED** completed 2026-09-14T22:43:40Z. AC-GE1-1…24 **PASS**.
>
> This section remains the testable engineering contract (AC-GE1-1…24), now reconciled to `main` after #105. This is an **additive** Company master UX/schema gap-edit on top of Wave 4 masters. Do **not** reopen Waves 1–5 as incomplete. Do **not** rewrite Wave 5 exit as incomplete. Do **not** claim the **module** COMPLETE from this recon (hotel UAT still required). **OPERATIONALLY ACCEPTED** / closed (Rekik formal closure YES / Advisor Outcome Review). Issue [#103](https://github.com/NORUDEVGIT/NORU/issues/103) **CLOSED** completed 2026-09-14T22:43:40Z. Production `0055` Abel/PM gated.
>
> Wave 4 delivered a **thin** Company master (required display name; optional code, phone, email, address, city, country, notes; status). That thin register **remains accepted**. Gap-edit 1 **enriched** Company only. Group / TA forms stay thin.
...
Grounded in `main` at documentation time (`6b6ac0e`, after PR [#105](https://github.com/NORUDEVGIT/NORU/pull/105) Company enrichment merge + [#104](https://github.com/NORUDEVGIT/NORU/pull/104) Spec §8 + [#102](https://github.com/NORUDEVGIT/NORU/pull/102) Wave 5 recon). **Code wins.** Inspected: `guest-company-form-dialog.tsx`, `guest-company-guest-links.tsx`, `guest-account-detail.tsx`, `guest-relationships-card.tsx`, `guest-accounts.functions.ts`, `guest-profile-company.ts`, `0055_pms_company_registration_enrichment.sql`.

| Surface | CURRENT |
|---|---|
| **Company form** | **LIVE** sectioned `GuestCompanyFormDialog` (`data-testid="company-form-sections"`). Six collapsible sections: **Basic** (default **open**), Tax & registration, Contact, Address, Commercial, Notes (others collapsed). Required: legal / company name (`name`) + company type enum `private_limited` \| `plc` \| `sole_proprietorship` \| `partnership` \| `ngo` \| `government` \| `other` (+ `company_type_other` when `other`). Optional: trade / display name, code, status, tax ID / TIN, business registration number, primary + alt phone/email (`phone` / `phone_alt` / `email` / `email_alt`), primary contact person, address line1 / line2 / city / region / country / postal, corporate account reference, negotiated rate reference (**name/code text only**), default TA FK, source of business, notes. |
| **Default TA** | **LIVE** nullable `default_travel_agent_master_id` FK to an existing same-property Travel Agent master. **Not** a `guest_account_links` row. Clearing the picker is allowed. |
| **Company detail** | `GuestAccountDetail` shows the enrichment field set + Edit + master history. **Link guests** (`GuestCompanyGuestLinks`) is LIVE on a **saved** Company: Directory search + checkboxes, role `employer` (default) \| `bill_to`, confirm via `linkGuestAccountsBulk` → `guest_account_links`. List linked guests + unlink (`unlinkGuestAccount`). Parties remain. |
| **Relationships** | Individual `GuestRelationshipsCard` **remains** LIVE. Single search + single-select + one role + **Link relationship**. Roles for Company: `employer` / `bill_to`. Unlink deletes the link row only. Company-side multi-link and individual single-link write the **same** store. |
| **Persistence** | Wave 4 columns plus 15 additive enrichment columns on `guest_account_masters` (`0055`): `trade_name`, `company_type`, `company_type_other`, `tax_id`, `business_registration_number`, `phone_alt`, `email_alt`, `primary_contact_name`, `address_line2`, `region`, `postal_code`, `corporate_account_reference`, `negotiated_rate_reference`, `default_travel_agent_master_id`, `source_of_business`. `guest_account_links` roles unchanged. Writers: `createGuestAccount` / `updateGuestAccount` / `linkGuestAccount` / `linkGuestAccountsBulk` / `unlinkGuestAccount`. Degrade: `COMPANY_ENRICHMENT_UNAVAILABLE` until 0055 apply. |
| **Group / TA forms** | Stay the Wave 4 **thin** `GuestAccountFormDialog`. Company-only sections are absent there. |
| **Individual Position / Department** | **Not** on `GuestFormDialog` / `GuestProfile`. **Not** on the Company form. |
| **Rate / folio / import** | **No** Company rate engine. Negotiated rate is name/code **text only**. Folio tab is **not** a Company surface. Import is **not** a Guest product. AC-W4-5 create-reservation residual **stands** (attach masters on reservation **detail**). |
| **Migration 0055** | Dual-lane `0055_pms_company_registration_enrichment.sql`. Non-prod **APPLY PASS** on `qcwptraosaudcbjasmul` (version `20260914221747`, 15 enrichment columns). **Production NOT applied** (Abel / PM gated). Prior residuals **untouched:** AC-W4-5; production 0053; production 0054. |

#### Must-not-claim (CURRENT)

- Waves 1–5 are **not** reopened as incomplete. Wave 4 thin Company register **was** accepted; Gap-edit 1 **enriched** Company only.
- Group / TA forms stay thin — this is **not** Group/TA enrichment.
- Individual Relationships remains. Multi-link is **not** a second relationship system.
...
- **No** rate engine. **No** Position / Department on Company.
- This recon does **not** claim the **module** COMPLETE.
...
| Contact | Alternative phone | `phoneAlt` | `phone_alt` | No | Additive (code wins vs earlier Spec draft `alt_phone`) |
| Contact | Business / primary email | `email` | `email` | No | Reuse Wave 4 email (same validation) |
| Contact | Alternative email | `emailAlt` | `email_alt` | No | Additive (same email validation if present; code wins vs earlier Spec draft `alt_email`) |
...
| **Non-prod apply** | **APPLY PASS** on `qcwptraosaudcbjasmul` — version `20260914221747` / `pms_company_registration_enrichment` (15 enrichment columns). |
| **Production apply** | **0055 Abel / PM gated. NOT applied.** Same hold as 0051 / 0053 / 0054. Do **not** treat Spec merge or Eng code merge as production schema apply. |
...
`NOT RUN` is never `PASS`. AC-GE1-1…24 results after #105 are recorded below the table.
...
#### Gap-edit 1 AC results (DER after merge)

DESIGN COMPLETION: **COMPLETE** (Gap-edit 1). IMPLEMENTATION STATUS: **PASS**. Approved deviations: **ONE** (Developer browser PARTIAL).

| ID | Result |
|---|---|
| **AC-GE1-1** | **IMPLEMENTED AS SPECIFIED / PASS** — Sectioned Company modal; Basic open; other sections collapsed |
| **AC-GE1-2** | **IMPLEMENTED AS SPECIFIED / PASS** — Legal name + company type required |
| **AC-GE1-3** | **IMPLEMENTED AS SPECIFIED / PASS** — Trade name / code persist; Directory search includes trade name |
| **AC-GE1-4** | **IMPLEMENTED AS SPECIFIED / PASS** — `other` requires other text |
| **AC-GE1-5** | **IMPLEMENTED AS SPECIFIED / PASS** — Tax & registration persist |
| **AC-GE1-6** | **IMPLEMENTED AS SPECIFIED / PASS** — Contact persist (incl. alt email validation) |
| **AC-GE1-7** | **IMPLEMENTED AS SPECIFIED / PASS** — Address persist (line2 / region / postal) |
| **AC-GE1-8** | **IMPLEMENTED AS SPECIFIED / PASS** — Commercial text persist; no rate engine |
| **AC-GE1-9** | **IMPLEMENTED AS SPECIFIED / PASS** — Default TA is existing TA master FK, not a link row |
| **AC-GE1-10** | **IMPLEMENTED AS SPECIFIED / PASS** — Notes persist |
| **AC-GE1-11** | **IMPLEMENTED AS SPECIFIED / PASS** — Link guests on saved Company detail only |
| **AC-GE1-12** | **IMPLEMENTED AS SPECIFIED / PASS** — Multi-select Directory guests; employer default / bill-to |
| **AC-GE1-13** | **IMPLEMENTED AS SPECIFIED / PASS** — Writes `guest_account_links` only (`linkGuestAccountsBulk`) |
| **AC-GE1-14** | **IMPLEMENTED AS SPECIFIED / PASS** — List + unlink; parties remain |
| **AC-GE1-15** | **IMPLEMENTED AS SPECIFIED / PASS** — Individual Relationships card remains |
| **AC-GE1-16** | **IMPLEMENTED AS SPECIFIED / PASS** — No Position / Department on Company |
| **AC-GE1-17** | **IMPLEMENTED AS SPECIFIED / PASS** — No rate engine / folio routing / Import / loyalty points |
| **AC-GE1-18** | **IMPLEMENTED AS SPECIFIED / PASS** — Group / TA forms stay thin |
| **AC-GE1-19** | **IMPLEMENTED AS SPECIFIED / PASS** — `pms` + `requireGuestManager` gate |
| **AC-GE1-20** | **IMPLEMENTED AS SPECIFIED / PASS** — Tenant-scoped |
| **AC-GE1-21** | **IMPLEMENTED AS SPECIFIED / PASS** — No entitlement / RLS **model** change |
| **AC-GE1-22** | **IMPLEMENTED AS SPECIFIED / PASS** — Waves 1–5 not reopened; AC-W4-5 not boiled in |
| **AC-GE1-23** | **IMPLEMENTED AS SPECIFIED / PASS** — Non-prod 0055 APPLY PASS; production 0055 Abel/PM gated; honest degrade |
| **AC-GE1-24** | **IMPLEMENTED AS SPECIFIED / PASS** — Module not claimed COMPLETE |

#### Gap-edit 1 approved deviations (DER / honesty)

| # | Deviation | Class |
|---|---|---|
| 1 | Developer browser QA remained **PARTIAL** | Independent QA covered Gap-edit 1 on non-prod after 0055 apply. Developer PARTIAL does not become PASS. |

#### Gap-edit 1 QA lanes recorded

| Lane | Result | Notes |
|---|---|---|
| Developer QA | **PARTIAL** | `tsc --noEmit` PASS; Wave 1–5 + AC-GE1 locks **PASS** (107/107). Browser QA-GE1 / SEC-GE1 **NOT RUN** in the developer environment. |
| Independent QA | **PASS** | Rekik via Advisor — [issue #103 Independent QA](https://github.com/NORUDEVGIT/NORU/issues/103#issuecomment-5671614352). |

`NOT RUN` is never `PASS`. Developer PARTIAL does not become PASS because Independent QA later passed.

#### Gap-edit 1 residuals / FINAL (not defects)

- Receptionist vs owner/manager RLS inconsistency remains **PRESERVED**.
- Production migration `0055_pms_company_registration_enrichment` **Abel-gated NOT applied**. Non-prod **APPLY PASS** on `qcwptraosaudcbjasmul` (version `20260914221747`, 15 enrichment columns).
- **Prior residuals untouched:** AC-W4-5 create-reservation master IDs; production 0053 Abel-gated; production 0054 Abel-gated.
- **Honesty:** no rate engine; no Position / Department on Company; Group / TA forms stay thin; Waves 1–5 stay closed.
- **OPERATIONALLY ACCEPTED** / closed (Rekik formal closure YES / Advisor Outcome Review). Issue [#103](https://github.com/NORUDEVGIT/NORU/issues/103) **CLOSED** completed 2026-09-14T22:43:40Z.
- DESIGN COMPLETION: **COMPLETE** (Gap-edit 1). IMPLEMENTATION STATUS: **PASS**. The module is **not** COMPLETE.
...
Gap-edit 1 **exited for engineering-gate purposes** after:

1. Rekik **approval** of this Spec — **DONE** 2026-09-14 via Hospitality Product Advisor ([#104](https://github.com/NORUDEVGIT/NORU/pull/104) MERGED)
2. Rekik **tech-plan approval** (issue [#103](https://github.com/NORUDEVGIT/NORU/issues/103)) — **DONE** 2026-09-14
3. Implementation against this §8 contract (extend Wave 4 masters / links — do **not** restart; do **not** reopen Waves 1–5) — **DONE** via PR [#105](https://github.com/NORUDEVGIT/NORU/pull/105)
4. Developer QA recorded — **PARTIAL** (`tsc` + locks 107/107 PASS; browser **NOT RUN**)
5. Independent QA **PASS** (Rekik via Advisor — [issue #103](https://github.com/NORUDEVGIT/NORU/issues/103#issuecomment-5671614352))
6. Human merge of PR [#105](https://github.com/NORUDEVGIT/NORU/pull/105) — **MERGED** 2026-09-14T18:55:43Z (Relates #103 — did not Closes)
7. Design Execution Report — **DONE**
8. Docs reconciliation to `main` (this recon; code wins)
9. Advisor Outcome Review — **OPERATIONALLY ACCEPTED**. Rekik formal closure — **DONE**. Issue [#103](https://github.com/NORUDEVGIT/NORU/issues/103) **CLOSED** completed 2026-09-14T22:43:40Z

Migration `0055` remains **Abel / PM gated** for production. Non-prod 0055 **APPLY PASS** on `qcwptraosaudcbjasmul` (version `20260914221747`, 15 enrichment columns).

**Hotel UAT** was **not** required to start Gap-edit 1 engineering. Hotel UAT **is** required for **module COMPLETE**.

Gap-edit 1 is **IMPLEMENTED ON MAIN**. ENGINEERING STATUS **COMPLETE / MERGED**. IMPLEMENTATION STATUS **PASS**. **OPERATIONALLY ACCEPTED** / closed (Rekik formal closure YES / Advisor Outcome Review). Issue [#103](https://github.com/NORUDEVGIT/NORU/issues/103) **CLOSED** completed 2026-09-14T22:43:40Z. This recon does **not** claim Waves 1–5 incomplete. The module is **not** COMPLETE.

Exit product summary: sectioned Company form LIVE (Basic open); required legal name + company type (+ other text when Other); tax / contact / address / commercial / notes LIVE; default TA FK LIVE; multi-select Directory link / list / unlink from Company detail via `guest_account_links` LIVE; Individual Relationships remains; no Position/Department on Company; no rate engine; Group/TA forms stay thin; tenant/auth honesty; production migration `0055` Abel-gated.
...
| **PACKAGE** | PMS |
| **PMS AREA** | Guests |
| **SPEC STATUS** | **ACCEPTED** + **IMPLEMENTED ON MAIN**. HOLD amend [#110](https://github.com/NORUDEVGIT/NORU/pull/110) MERGED (AC-GE2-9…14). Linking Scope C **REQUIRED** on New Guest **before** submit — **delivered**. |
| **ENGINEERING STATUS** | **COMPLETE / MERGED**. IMPLEMENTATION STATUS **PASS**. HOLD **cleared** via [#112](https://github.com/NORUDEVGIT/NORU/pull/112). **OPERATIONALLY ACCEPTED** / closed (Rekik formal closure YES / Advisor Outcome Review). PR [#111](https://github.com/NORUDEVGIT/NORU/pull/111) MERGED 2026-09-14T23:07:06Z (enrichment; early, without staged Create) · PR [#112](https://github.com/NORUDEVGIT/NORU/pull/112) MERGED 2026-09-14T23:30:45Z (staged Create HOLD fix). Issue [#109](https://github.com/NORUDEVGIT/NORU/issues/109) **CLOSED** completed 2026-09-14T23:46:37Z (Rekik formal closure YES / Advisor Outcome Review). Non-prod `0057` **APPLY PASS** on `qcwptraosaudcbjasmul` (version `20260914234317` / `pms_individual_form_enrichment`; 16/16 columns; `guest_emergency_contacts` YES). **Production 0057 NOT applied** (Abel-gated). |
| **PERMISSIONS** | Package entitlement **pms** + existing guest manage gate (`getGuestsAccess` / `requireGuestManager`). No new entitlement model unless Abel-flagged. Receptionist residual **PRESERVED**. Additive RLS that **matches** existing guest-table roles is expected and is **not** a model change. |
...
| **REQUIREMENTS** | **Locked** from the product requirement (Rekik APPROVED 2026-09-15) + **HOLD amendment** (Rekik 2026-09-15): staged-create on New Guest (**AC-GE2-9…14**) — **delivered** on `main` via #112 |

> **Gap-edit 2 IMPLEMENTED ON MAIN.** ENGINEERING STATUS **COMPLETE / MERGED**. IMPLEMENTATION STATUS **PASS**. HOLD **cleared** via [#112](https://github.com/NORUDEVGIT/NORU/pull/112). Independent QA **PASS** on #112. AC-GE2-1…34 **PASS**. Locks **142/142 PASS**. Developer browser **NOT RUN**. **OPERATIONALLY ACCEPTED** / closed (Rekik formal closure YES / Advisor Outcome Review).
>
> This section remains the testable engineering contract (**AC-GE2-1…34**, Linking create-path = **AC-GE2-9…13**, staged Identity = **AC-GE2-14**), now reconciled to `main` after #111 + #112. Do **not** reopen Waves 1–5 as incomplete. Do **not** reopen or rewrite Gap-edit 1 (§8). Do **not** claim the **module** COMPLETE (hotel UAT still required). Issue [#109](https://github.com/NORUDEVGIT/NORU/issues/109) is **CLOSED** completed 2026-09-14T23:46:37Z (Rekik formal closure YES / Advisor Outcome Review). Non-prod `0057` **APPLY PASS** on `qcwptraosaudcbjasmul` (version `20260914234317`). **Production 0057 NOT applied** (Abel-gated).
>
> Wave 1–2 delivered a **flat** Individual `GuestFormDialog` plus a **separate** Identity & Documents **card**. That contract **remains accepted**. Gap-edit 2 **enriches** the Individual create/edit path (including **staged-create**), adds a front-desk restriction flag, and adds Individual-side **Linking / Relationships** that reuses Wave 4 `guest_account_links`. **After-save-only on create fails Spec** and is **not** CURRENT after #112.

### 9.1 Business purpose

Give authorised staff a **sectioned Individual create/edit modal** (basic identity, contact, address, collocated ID upload, employment, emergency contacts, light commercial notes), a **blacklist / restricted** product flag that is obvious on Directory and Information, and an Individual-side **Linking / Relationships** section that attaches existing Company / Group / Travel Agent masters **on New Guest / Create** (staged) **and** after save — so front desk can register a complete individual, attach ID files and commercial links in **one Create**, see a clear stay-path warning, and keep Relationships in sync without inventing KYC, police export, Import, folio routing, a guest↔guest family graph, or new reservation / Front Office products. **After-save-only on create is no longer the product bar.**

Gap-edit 2 is Guest-owned **Individual enrichment**. It is **not** Company further edits, **not** Group/TA form enrichment, **not** Import, **not** a Folio tab, **not** loyalty points, **not** rich Comms, **not** AC-W4-5 create-reservation boil-in, **not** government KYC, **not** a guest-to-guest family graph, and **not** a Wave 1–5 reopen.

### 9.2 CURRENT behaviour (code wins)

Grounded in `main` at `d6058d5` (after [#111](https://github.com/NORUDEVGIT/NORU/pull/111) Individual enrichment + [#112](https://github.com/NORUDEVGIT/NORU/pull/112) staged Create + [#110](https://github.com/NORUDEVGIT/NORU/pull/110) Spec HOLD amend). **Code wins.** Inspected: `guest-form-dialog.tsx`, `guest-form-staged-identity.tsx`, `guest-form-staged-links.tsx`, `guest-profile-individual.ts`, `guest-detail-workspace.tsx`, `guest-identity-card.tsx`, `guest-bits.tsx`, `guest-directory-workspace.tsx`, `guest-relationships-card.tsx`, `guest-accounts.functions.ts`, `guests.functions.ts`, `guest-privacy.functions.ts`, FO / reservation warn hooks.

| Surface | CURRENT after #111 + #112 |
|---|---|
| **Individual create / edit** | **Sectioned** `GuestFormDialog` **LIVE**. Sections: Basic (open by default), Contact, Address, Identity, Employment, Emergency, Notes / commercial light, Restriction, Linking. First name still required; walk-in first-name-only still works. Title, middle, preferred, gender, alt phone/email, status, Position / Department, source of business persist. |
| **Identity upload** | **Collocated LIVE** on form Identity + Information Identity. Reuses Wave 2 `guest_documents` — **no second store**. Identity **card remains**. On **New Guest**, files are **staged** until Create (`individual-identity-staged`). On Edit, upload is after-save. |
| **Staged Create** | **LIVE** (#112 HOLD cleared). One Create: (1) insert guest (2) upload staged files via `guest_documents` (3) write staged masters via `guest_account_links` (`employer` \| `bill_to` \| `booker_ta` \| `group_member`) (4) open profile when complete. **Partial-failure honesty:** if upload or link fails after insert, guest **exists**; `individual-create-partial-failure` + Retry remaining + Open guest. **Never** silent full success. After-save-only on create **fails Spec** and is **not** CURRENT. |
| **Information** | Overview panels plus **restriction warn** + badges. Identity panel can start the same Wave 2 upload. History tab is profile events (includes restriction set/clear), not stay history. |
| **Identity & Documents card** | **LIVE** (Wave 2, not reopened): same `guest_documents` store. Staff verify / reject with actor + time. Copy remains staff confirmation only — **not** KYC. Card lists documents uploaded from the form / Information. |
| **Directory** | `listGuests` + VIP / status **and** restricted / blacklisted **badges** (`GuestRestrictionBadges`). Restricted guests stay findable. |
| **Restriction** | **LIVE.** Flags `restricted` and/or `blacklisted` + optional `restriction_severity` (`watch` \| `elevated` \| `severe`) + required reason + by / at + optional until. Set / lift UI with confirm + reason. History `restriction_set` / clear. **Warn** (not hard block) on existing stay paths: reservation detail, FO dialogs / amend / walk-in / booking, Information. Copy: stay create is not hard-blocked. |
| **Emergency / employment** | **LIVE** on the individual. Normalized `guest_emergency_contacts` (add / remove; empty allowed on walk-in). Position / Department on the individual only (Company still has none — AC-GE1-16). |
| **Stay paths that already touch this guest** | `GuestRestrictionWarn` on reservation detail, FO guest pick / amend / walk-in / booking, and Information. `create_hotel_reservation_priced` still takes `_guest_id` only. AC-W4-5 residual **stands**. **No** new Res / FO product. **No** hard block. |
| **Wave 2 duplicate warn** | Open existing / Create anyway / optional Merge CTA — **no** Compare UI. |
| **Linking / Relationships** | **Staged on Create + after-save LIVE.** New Guest Linking section stages Company / Group / TA + role (`individual-linking-staged`) — no write until insert. After save / on Information, list + unlink via Wave 4 `guest_account_links`. `GuestRelationshipsCard` stays in sync (same store). Gap-edit 1 Company multi-link is a **separate** Company-side batch (§8) — **not** rewritten here. |
| **Migrations on `main`** | `0051` Wave 2 · `0053` Wave 4 · `0054` Wave 5 · `0055` Company enrichment · `0056` Polish Wave 1 · **`0057_pms_individual_form_enrichment.sql` on `main` from #111**. Non-prod **APPLY PASS** on `qcwptraosaudcbjasmul` (version `20260914234317` / `pms_individual_form_enrichment`; 16/16 enrichment+restriction columns; `guest_emergency_contacts` YES). **Production 0057 NOT applied** (Abel-gated). Production surfaces that need the new columns / `guest_emergency_contacts` degrade to `INDIVIDUAL_ENRICHMENT_UNAVAILABLE` until apply. Do **not** overwrite Polish `0056`. Do **not** claim production 0057 applied. |

#### Must-not-claim (CURRENT)

- Wave 1–2 Individual Directory / Information / Identity **card** contracts remain accepted — this batch **enriched** them; it did **not** reopen them as incomplete.
- Wave 2 Identity **card** upload / mask / staff verify is **LIVE** and **accepted**. Gap-edit 2 **collocates** upload; it did **not** invent a second document store.
- Gap-edit 1 Company enrichment is a **separate** batch (§8). This recon does **not** rewrite §8.
- Staff verify is **not** government KYC and must not become that.
- **Production `0057` is not applied.** Code merge ≠ production schema apply. Non-prod **APPLY PASS** on `qcwptraosaudcbjasmul` (`20260914234317` / `pms_individual_form_enrichment`; 16/16 columns; `guest_emergency_contacts` YES). Production remains Abel-gated.
- Staged Create is **LIVE**. After-save-only on create is **not** CURRENT and **fails Spec**.
- This recon does **not** claim the **module** COMPLETE.
...
| **8. Linking / Relationships** | Collapsed · **staged on Create** + after save | Staff may pick Company / Group / TA + role **before** first submit. One **Create** writes Wave 4 `guest_account_links` after the guest row exists. Edit-after-save Linking **remains**. See **Scope C** and **Staged create**. |
...
| **Create vs edit** | Wave 2 APIs require a `guestId`. On **New Guest**, staff **must** be able to **stage ID file(s) before submit**. One **Create** inserts the guest, then uploads staged files via Wave 2 `guest_documents`. **Eng-chooses save-first-only is no longer enough.** Edit-after-save upload **remains**. **No silent drop.** If insert succeeds but upload fails, the guest **exists** — show **retry**, never silent full success. |
...
#### Scope C — Linking / Relationships (Rekik amendment 2026-09-15; staged-create HOLD 2026-09-15)

Individual-side counterpart to Gap-edit 1’s Company multi-link. **Reuse Wave 4 `guest_account_links` only.** **Staged-create on New Guest is the product bar.** After-save-only on create is **no longer** enough. Edit-after-save Linking **remains**.

| Behaviour | Expected |
|---|---|
| **Where** | A **Linking / Relationships** section on Individual **create and edit**. On **New Guest**, staff **may** search + pick Company / Group / TA + role **before** submit (staged). After first save, Linking stays available (edit-after-save). **Do not** hide Linking on unsaved New Guest. |
| **Pick** | Search + pick an **existing** same-property Company, Group, or Travel Agent master (`listGuestAccounts`). Do **not** create a master from this section. |
| **Roles** | Reuse Wave 4 only: `employer` \| `bill_to` \| `booker_ta` \| `group_member`. Role must match master type (`ROLE_ACCOUNT_TYPE`: employer/bill-to → Company; `booker_ta` → Travel Agent; `group_member` → Group). **No new roles.** |
| **Write** | On Create: staged picks write **after** the guest row exists, via `linkGuestAccount` (or a thin wrapper that inserts the **same** rows + history). On edit: confirm writes immediately. Unique `(restaurant_id, guest_id, master_id, role)` stands. Duplicate pair: reject or skip honestly. **Do not** invent a pre-id link table. |
| **List + unlink** | After create / on edit, section lists linked masters + role. Unlink uses `unlinkGuestAccount` — **deletes the link only**; individual and master **remain**. |
| **Relationships card** | **Remains.** Links created here (including staged-create) appear on `GuestRelationshipsCard` and on the master side. Links created on the card appear here. **Same store.** |
| **Company multi-link** | Gap-edit 1 Company-side linker **remains** and is **not** rewritten. This is the Individual-side counterpart. |
| **Not** | **No** second link table. **No** folio routing invent (`transfersSupported: false` stands). **No** guest↔guest family graph. **No** AC-W4-5 create-reservation boil-in. |

#### Staged create (Rekik HOLD 2026-09-15)

On **New Guest / Create**, staff may attach ID file(s) **and** choose Company / Group / TA + role **before** submit. One **Create** must, under the hood:

1. **Create the guest** (same validation — first name required, walk-in first-name-only still works).
2. **If files staged:** upload via Wave 2 `guest_documents` (no second store).
3. **If masters / roles chosen:** write Wave 4 `guest_account_links` (`employer` \| `bill_to` \| `booker_ta` \| `group_member`).
4. **Navigate** to profile / success with **links + docs visible**.

**Delivered on `main` via [#112](https://github.com/NORUDEVGIT/NORU/pull/112)** after [#111](https://github.com/NORUDEVGIT/NORU/pull/111) merged early without this path. Independent QA **PASS** on #112. HOLD **cleared**.

**Honesty on partial failure:** If create succeeds but upload or link fails, the guest **still exists**. Show **clear retry UI** (`individual-create-partial-failure` + Retry remaining + Open guest). **Never** silent full success.

Edit-after-save Linking + Identity upload paths **remain**. **After-save-only on create fails Spec** and is **not** CURRENT.

| CURRENT vs EXPECTED | |
|---|---|
| **CURRENT (`main` after #111 + #112)** | Staged ID files + staged Linking **on Create** (create → `guest_documents` → `guest_account_links`); partial-failure retry; edit-after-save remains. After-save-only on create is **not** CURRENT. |
| **EXPECTED (this batch)** | Same — **delivered**. Hotel UAT is **module** DoD, not a Gap-edit 2 reopen. |
...
| **Wave 4 Relationships** | Individual Relationships **card remains** and stays in sync with Scope C (same `guest_account_links` store). Do **not** hide or replace the card. |
...
| **Scope C links** | Reuse `guest_account_links` / `linkGuestAccount` / `unlinkGuestAccount`. No second table. No folio routing. No guest↔guest family. |
...
| Part | EXPECTED |
|---|---|
| **(A) Sectioned Individual form** | Seven collapsible profile sections; Basic open by default; first name still required; address stays in Address; Identity upload collocated via Wave 2 APIs; employment on individual; emergency add/remove; notes + optional source of business. |
...
| **(C) Linking / Relationships** | On **New Guest**, staff may stage Company / Group / TA + role **before** submit. One **Create** writes Wave 4 `guest_account_links` after the guest exists. Edit-after-save Linking **remains**. Relationships card stays in sync. No second table; no folio routing; no guest↔guest family. Partial upload/link failure after create → retry UI, never silent full success. |
| **(D) Honesty** | No second document store. No entitlement-model change unless Abel-flagged. Waves 1–5 and Gap-edit 1 not reopened. Module not COMPLETE. Hard block only with Abel. |
...
| Linking | Linked masters + role | Wave 4 `linkGuestAccount` | `guest_account_links` | Staged on Create + after save | **Reuse Wave 4** — no new columns. Roles `employer` \| `bill_to` \| `booker_ta` \| `group_member`. Staged picks write after guest insert. |
...
**Not in this inventory:** Import mapping, folio routing, loyalty points, Compare-duplicates UI, Company/Group/TA form fields, police export payload, guest↔guest family edges.
...
### 9.6a Linking / Relationships rules (Scope C)

| Rule | Expected |
|---|---|
| **When** | On **New Guest / Create** (stage Company / Group / TA + role **before** submit) **and** after the guest is saved (edit-after-save). Linking is **not** hidden on unsaved New Guest. Staged picks do **not** write until Create inserts the guest. |
| **Who** | Same-property Company / Group / Travel Agent masters (`listGuestAccounts`). Retired / anonymised masters follow existing Wave 4–5 list honesty. |
| **Select** | Search + pick one master, choose a Wave 4 role that matches that master’s type, confirm. Confirm with no master selected is an honest no-op or validation — **not** a silent success. Multi-pick in one confirm is OK if simple; not required. |
| **Role map** | `employer` / `bill_to` → Company; `booker_ta` → Travel Agent; `group_member` → Group. A mismatched type is rejected with the existing Wave 4 `assertRoleMatchesType` honesty. |
| **Write** | One `guest_account_links` row per guest + master + role. Reuse `linkGuestAccount`. History `relationship_linked` on guest + master (Wave 4). |
| **Visible both sides** | Individual Linking section **and** Relationships card **and** the master’s Relationships / Gap-edit 1 Company list (AC-W4-10 / AC-GE1-14 stand). |
| **Unlink** | Per link, existing `unlinkGuestAccount`. Individual and master remain. History `relationship_unlinked`. |
| **Schema** | **No** new links table. Scope C does **not** by itself require `0057` columns. |
...
| **File** | Authorized: dual-lane **`0057`** — additive `guest_profiles` columns + `guest_emergency_contacts`. Mirror `supabase/migrations/` and `drizzle/migrations/`. **Honesty:** Eng used **0057** because `main` already has Polish Wave 1 `0056_pms_polish1_payment_methods_admin_fee_presets.sql`. Do **not** overwrite that file. Do **not** claim the GE2 gate is still 0056. Non-prod **APPLY PASS**; production Abel-gated. |
...
| **Non-prod apply** | **APPLY PASS** on `qcwptraosaudcbjasmul` — version `20260914234317` / `pms_individual_form_enrichment`. 16/16 enrichment+restriction columns; `guest_emergency_contacts` YES. |
| **Production apply** | **0057 Abel / PM gated — NOT applied.** Same hold as production 0051 / 0053 / 0054 / 0055. Do **not** treat Spec merge, Eng code merge, or non-prod apply as production schema apply. |
| **Degrade** | Surfaces that need the new columns / table show an honest unavailable message until apply **on that environment** (follow `WAVE2_MIGRATION_UNAVAILABLE` / Wave 4–5 pattern). Token `INDIVIDUAL_ENRICHMENT_UNAVAILABLE`. Non-prod is applied; production still degrades. |
...
| Guest-to-guest family graph | Later — **not** this batch. Linking is individual → existing Company / Group / TA masters only |
| Folio split / routing from a link | Cashiering / later. Bill-to remains an **association** (`transfersSupported: false`) |
...
| Production migration `0057` apply | Abel / PM — **NOT applied**; not this Spec. Non-prod APPLY PASS is **not** production apply. (`0056` is Polish Wave 1 on `main` — do not overwrite.) |
| Entitlement / RLS **model** change | Flag Abel; additive matching RLS is OK |
| Reopening Waves 1–5 as incomplete | Forbidden |
...
`NOT RUN` is never `PASS`. AC results after #111 + #112 DER are in **Gap-edit 2 AC results** below.

| ID | Criterion | Pass |
|---|---|---|
| **AC-GE2-1** | Individual create / edit uses a **sectioned** modal (Basic, Contact, Address, Identity, Employment, Emergency, Notes / commercial light). Linking / Relationships is **Scope C** (staged on Create + after save). | Open New Guest and Edit guest. Seven named profile sections are present. Basic is **expanded** by default. Other profile sections are **collapsed** by default and can be opened. Linking is **available on unsaved New Guest** (stage picks) and after save (**AC-GE2-9**). Company / Group / TA dialogs are not restyled by this batch. |
...
| **AC-GE2-9** | On **New Guest**, staff may **search + pick** existing same-property Company / Group / Travel Agent masters **and a role before submit**. Edit-after-save Linking **remains**. | Unsaved New Guest can **stage** a master + role (Linking is **not** hidden). Staged picks do **not** write a link until Create inserts the guest. After create or on Edit, staff can still search and pick. Other-property masters are not listed / are rejected. A type-mismatched role (e.g. Group + `employer`) is rejected. **After-save-only on create fails this AC.** |
| **AC-GE2-10** | One **Create** (and edit confirm) **writes `guest_account_links`** with a Wave 4 role only: `employer` \| `bill_to` \| `booker_ta` \| `group_member`. | Staged Company + `employer` or `bill_to` on Create → link row after guest insert. TA + `booker_ta` and Group + `group_member` succeed. Diff adds **no** peer links table and **no** pre-id link table. History matches Wave 4 (`relationship_linked`). Duplicate guest+master+role is rejected or skipped honestly. If guest create succeeds but the link write fails, show **retry** — never silent full success. |
| **AC-GE2-11** | After create / on edit, the section **lists** linked masters + role and can **unlink** without deleting the guest or master. | After staged-create, profile / success shows the link. List shows master name + type + role. Unlink → row gone; individual still in Individual Directory; Company / Group / TA still in their Directory. |
| **AC-GE2-12** | **Relationships card remains** and shows the **same links** (same store; no second system), including links written by staged-create. | A link created in Scope C (Create or edit) appears on `GuestRelationshipsCard`. A link created on the card appears in the Linking section. Gap-edit 1 Company multi-link still works and is **not** rewritten. |
| **AC-GE2-13** | Scope C create-path and edit-path do **not** invent a guest↔guest family graph, folio split / routing, or AC-W4-5 boil-in. | No individual-to-individual family picker. No “folio routed” success. `transfersSupported: false` unchanged. `create_hotel_reservation_priced` still does not take master IDs. Staged-create does not invent a second store. |
| **AC-GE2-14** | Create-path Identity upload is **staged**: staff attach file(s) **before** submit; one **Create** uploads via Wave 2 after guest insert. | Creating a guest **and** attaching a file from the form succeeds without a silent “upload ignored”. The file is held until `createGuest` returns, then uploaded via `guest_documents` (no second store). Profile / success shows the document. **Save-first-only (hide upload until after save) fails this AC.** If insert succeeds but upload fails, the guest **exists** — show **clear retry UI**; never silent full success. Edit-after-save upload **remains**. |
| **AC-GE2-15** | Staff verify remains **staff confirmation only** — **no KYC claim**. | Verify / reject still uses Wave 2 review APIs + actor + time. Visible copy does **not** say government verification, KYC, or police check. `STAFF_VERIFY_COPY` honesty stands. |
| **AC-GE2-16** | **Employment** Position and Department persist on the **individual** and are **not** on Company. | Both save and reload on that guest. Company modal still has no Position / Department (AC-GE1-16). |
| **AC-GE2-17** | **Emergency** contacts: add / remove; fields persist; multi allowed. | Add one contact (name + phone or email); reload; row matches. Add a second; both remain. Remove one; the other remains. Empty Emergency on first-name-only create still succeeds. |
| **AC-GE2-18** | **Notes** and optional **source of business** persist. | Notes still use `notes`. Source of business saves and reloads. No rate-plan picker, no “source applied” commercial engine. |
| **AC-GE2-19** | Staff can set **restricted** or **blacklisted** only with a **reason**; server records **by + at**. | Set with blank reason → blocked. Set with reason → status persists; recorded by / at are populated from the session, not a client spoof. Optional until persists when provided. |
| **AC-GE2-20** | Restriction is **visible** on Directory and Information (and Identity / header). | Directory row shows a Restricted or Blacklisted badge (with VIP/status still visible). Information shows badge + alert including reason. Header / Identity area also shows the badge or the same alert. |
| **AC-GE2-21** | **History** records set and clear. | After set, Information History (or hub feed) shows a restriction-set event with reason. After lift, a restriction-cleared event with lift reason. Events are not stay-history rows. |
| **AC-GE2-22** | **Lift / clear** requires confirm + reason. | Lift without confirm does not clear. Lift with reason → status `none`; badge gone; guest still in Directory. Silent toggle is a fail. |
| **AC-GE2-23** | Stay paths that **already** touch this guest **warn** (minimum) if easy. | New reservation guest pick and FO check-in stepper (when that guest is loaded) show a visible restriction warning. No new reservation / FO module is created. AC-W4-5 residual is **not** boiled. |
| **AC-GE2-24** | **No hard block** unless Abel + Spec agree; prefer warn + strong UI. | Create-reservation / check-in still complete after the warning (staff can proceed). If Eng adds a hard block, the PR must record Abel agreement. If a path cannot warn without a new product, Eng records OUT-OF-SCOPE FINDING — not a silent miss presented as PASS. |
| **AC-GE2-25** | Gap-edit 2 does **not** invent Import, Folio tab, loyalty points, rich Comms, police export, or a KYC claim. | No import wizard, no folio-routing success, no points widget, no marketing send, no police-export download, no “government verified” copy. |
| **AC-GE2-26** | Denied staff cannot read or write enrichment fields, emergency contacts, documents, restriction state, or Scope C links. | Same property, role outside the working gate: no new PII, no restriction reason, no links. Other-property guest / master ids fail. Public / unauthenticated users redirect to login. |
| **AC-GE2-27** | Individual data, emergency contacts, documents, restriction, and links stay **tenant-scoped**. | Property 2 does not see property 1 guests, contacts, files, restriction reason, or `guest_account_links`. `restaurantId` from the client is not trusted alone. |
| **AC-GE2-28** | **No** entitlement / RLS **model** change unless Abel-flagged. | Diff has no new package and no new RLS role model. Additive RLS matching guest-table roles is OK. Receptionist residual **PRESERVED**. Hard-block RPC / security-definer in an exposed schema **flags Abel**. |
| **AC-GE2-29** | Waves 1–5 LIVE surfaces **remain**. Gap-edit 1 is **not** reopened or rewritten as incomplete. | Individual Directory / Information, Identity card, Preferences, merge, consent, Stay History, Dashboard, Loyalty, Relationships, Wave 5 hub / privacy (as documented on `main`), and Company / Group / TA masters still work on the same routes. §8 ACs are not edited by this section. AC-W4-5 residual is **not** boiled. |
| **AC-GE2-30** | Production schema apply is **not** claimed from this Spec or from Eng merge. | **Production `0057` Abel-gated NOT applied.** Non-prod **APPLY PASS** on `qcwptraosaudcbjasmul` (`20260914234317` / `pms_individual_form_enrichment`; 16/16 columns; `guest_emergency_contacts` YES). Production surfaces degrade honestly until apply. Production 0051 / 0053 / 0054 / 0055 holds are untouched. Do **not** overwrite Polish Wave 1 file `0056_pms_polish1_payment_methods_admin_fee_presets.sql` already on `main`. |
| **AC-GE2-31** | This gap-edit does **not** claim the **module COMPLETE**. | Copy / DER / PR do not mark Guest Profile COMPLETE. Hotel UAT remains required. |
| **AC-GE2-32** | Wave 5 export / anonymise **extend** to new Individual PII without failing Wave 5. | Export JSON includes new profile fields / emergency contacts / restriction reason (authorised privacy officer). Anonymise scrubs that PII. Wave 5 ACs stay PASS — this is an extension, not a reopen. |
| **AC-GE2-33** | Preferences stay on the Preferences card; consent stays on Information (+ Privacy); Identity **card** remains. | Edit guest does not become the Preferences editor. Consent panel still works. Identity card still lists / verifies the same `guest_documents`. |
| **AC-GE2-34** | Wave 1 duplicate **warn** remains; Compare is **not** required. | Email/phone match still offers Open existing / Create anyway / optional Merge. No silent merge. No new Compare product unless Eng lands a trivial extra and records it as approved deviation. |

#### Gap-edit 2 AC results (DER after #111 + #112)

DESIGN COMPLETION: **COMPLETE** (Gap-edit 2). IMPLEMENTATION STATUS: **PASS**. HOLD **cleared** via [#112](https://github.com/NORUDEVGIT/NORU/pull/112). Approved deviations: **TWO** (see below).

| ID | Result |
|---|---|
| **AC-GE2-1** | **IMPLEMENTED AS SPECIFIED / PASS** — Sectioned Individual form LIVE; Basic open; Linking staged on Create + after save |
| **AC-GE2-2** | **IMPLEMENTED AS SPECIFIED / PASS** — First name required; walk-in first-name-only still works |
| **AC-GE2-3** … **AC-GE2-6** | **IMPLEMENTED AS SPECIFIED / PASS** — Basic / Contact / Address / Identity text persist; mask remains |
| **AC-GE2-7** … **AC-GE2-8** | **IMPLEMENTED AS SPECIFIED / PASS** — Identity upload collocated on form + Information; Wave 2 `guest_documents` only |
| **AC-GE2-9** … **AC-GE2-13** | **IMPLEMENTED AS SPECIFIED / PASS** — Staged Linking on Create; one Create writes `guest_account_links`; list + unlink; Relationships card same store; no family / folio / AC-W4-5 / second store. After-save-only on create **fails** these ACs and is **not** CURRENT |
| **AC-GE2-14** | **IMPLEMENTED AS SPECIFIED / PASS** — Staged Identity files on Create; upload after insert; save-first-only fails; retry; edit-after-save remains |
| **AC-GE2-15** … **AC-GE2-18** | **IMPLEMENTED AS SPECIFIED / PASS** — Staff verify ≠ KYC; Employment on individual; Emergency add/remove; notes + source of business |
| **AC-GE2-19** … **AC-GE2-22** | **IMPLEMENTED AS SPECIFIED / PASS** — Restricted / blacklisted + reason + by/at; badges; history; lift with confirm + reason |
| **AC-GE2-23** … **AC-GE2-24** | **IMPLEMENTED AS SPECIFIED / PASS** — Stay-path **warn** LIVE (not hard block). No new Res / FO product |
| **AC-GE2-25** … **AC-GE2-34** | **IMPLEMENTED AS SPECIFIED / PASS** per DER / Independent QA — including production 0057 Abel-gated (AC-GE2-30; non-prod APPLY PASS `20260914234317`) and module not COMPLETE (AC-GE2-31) |

#### Gap-edit 2 approved deviations (DER / honesty)

| # | Deviation | Class |
|---|---|---|
| 1 | PR [#111](https://github.com/NORUDEVGIT/NORU/pull/111) merged **early** (2026-09-14T23:07:06Z) **without** staged Create. HOLD fix shipped as PR [#112](https://github.com/NORUDEVGIT/NORU/pull/112) MERGED 2026-09-14T23:30:45Z | **Process note only** — not a product defect. Staged Create is LIVE on `main`. HOLD **cleared**. |
| 2 | Developer browser QA remained **NOT RUN** / **PARTIAL** | Independent QA **PASS** on #112. `NOT RUN` is never PASS. Developer PARTIAL does not become PASS. |

#### Gap-edit 2 QA lanes recorded

| Lane | Result | Notes |
|---|---|---|
| Developer QA | **PARTIAL** | `tsc --noEmit` PASS; Guest Profile locks **142/142 PASS** (Waves 1–5 + Company AC-GE1-1…24 + Individual AC-GE2-1…34). Browser QA-GE2 / SEC-GE2 **NOT RUN** in the developer environment. |
| Independent QA | **PASS** | Rekik via Advisor on follow-up [#112](https://github.com/NORUDEVGIT/NORU/pull/112). Evidence: [issue #109 Independent QA / merge verified](https://github.com/NORUDEVGIT/NORU/issues/109#issuecomment-5672307748). HOLD **cleared**. |

`NOT RUN` is never `PASS`. Developer PARTIAL does not become PASS because Independent QA later passed.

#### Gap-edit 2 residuals / FINAL (not defects)

- Receptionist vs owner/manager RLS inconsistency remains **PRESERVED**. Additive GE2 RLS matches guest tables.
- Non-prod migration `0057_pms_individual_form_enrichment` **APPLY PASS** on `qcwptraosaudcbjasmul` (version `20260914234317`; 16/16 enrichment+restriction columns; `guest_emergency_contacts` YES). **Production 0057 Abel-gated NOT applied**. Production surfaces degrade honestly (`INDIVIDUAL_ENRICHMENT_UNAVAILABLE`) until apply. Do **not** claim production 0057 applied.
- Production `0051` / `0053` / `0054` / `0055` holds remain untouched. Polish `0056` file on `main` is not overwritten.
- **Wave 4 residual untouched:** AC-W4-5 create-reservation master IDs (attach on detail).
- **OPERATIONALLY ACCEPTED** / closed (Rekik formal closure YES / Advisor Outcome Review). Issue [#109](https://github.com/NORUDEVGIT/NORU/issues/109) **CLOSED** completed 2026-09-14T23:46:37Z (Rekik formal closure YES / Advisor Outcome Review).
- Programme remains **READY FOR HOTEL UAT**. Module COMPLETE remains **NO**.
- DESIGN COMPLETION: **COMPLETE** (Gap-edit 2). IMPLEMENTATION STATUS: **PASS**. The module is **not** COMPLETE.
- This recon does **not** reopen Waves 1–5 or rewrite Gap-edit 1 (§8).
...
| **QA-GE2-2** | Basic open by default; other profile sections collapsed; all seven profile sections can be opened. Linking **available on New Guest** (stage Company / Group / TA + role before submit). | Screenshot + note. After-save-only Linking on create is a **fail**. |
...
| **QA-GE2-5** | Identity upload from **form** and from **Information** Identity section; same document on Identity card; no second store. | Create-path **must** stage file(s) before submit; one Create uploads via Wave 2. If upload fails after create, retry UI — not silent success. |
...
| **QA-GE2-12** | **Staged-create:** New Guest → stage Company + employer → Create. Link + docs (if staged) visible on profile / Relationships card **and** Company detail. Also exercise edit-after-save Linking. Unlink; both parties remain. | Partial link failure after create → retry, not silent success. |
| **QA-GE2-13** | Link a TA as booker and a Group as member; type-mismatch role is rejected. No guest↔guest picker. | |
| **QA-GE2-14** | `tsc --noEmit` (or project equivalent) + lock tests on the implementation PR. | Developer lane. |
| **QA-GE2-15** | Independent QA after Developer QA — **PASS** on [#112](https://github.com/NORUDEVGIT/NORU/pull/112) (HOLD cleared). | Do **not** treat #111-alone as Independent QA PASS (merged early without staged Create). Hotel UAT is **module** DoD. |

### 9.11 Security (Gap-edit 2)

| ID | Check |
|---|---|
| **SEC-GE2-1** | Unauthenticated visit to Guest canonical routes (including Individual create / edit / restriction / Linking) redirects to login. |
| **SEC-GE2-2** | Membership **without** package `pms` cannot use enriched Individual fields, emergency contacts, restriction, or Scope C links. |
| **SEC-GE2-3** | Staff who fail `canManageGuests` / `requireGuestManager` cannot read or write enrichment, documents, restriction reason, or links. |
| **SEC-GE2-4** | Tenant isolation: new columns, emergency contacts, documents, restriction, and links re-derive `restaurantId` from membership. Property 1 guest / master is not returned for property 2. |
| **SEC-GE2-5** | No new public / customer route exposes ID numbers, emergency contacts, restriction reason, or employment. |
| **SEC-GE2-6** | Lift is not a silent clear. Restriction set is not a silent no-op presented as success when reason is blank. Linking confirm with zero masters selected is not a silent success. Unlink is not a silent delete of people or masters. Create succeeding while upload or link fails is **not** presented as full success. |
...
| **REG-GE2-4** | Wave 4 Company / Group / TA masters, Relationships both sides, honest Loyalty, and profile-type switcher still work. Scope C links use the same card/store. AC-W4-5 residual **untouched**. |
...
| **REG-GE2-6** | Gap-edit 1 Company sectioned form + multi-link **not** rewritten or treated as incomplete. Position / Department stay off Company. Company-side Link guests still works. |
| **REG-GE2-7** | Sales & Events placeholder remains planned. Cashiering `transfersSupported: false` unchanged. |
| **REG-GE2-8** | No new package; no second `guest_documents` store; no second relationship table; no guest↔guest family graph. |
...
| **REG-GE2-12** | Production 0051 / 0053 / 0054 Abel-gates remain; this gap-edit does not apply them. Non-prod **`0057` APPLY PASS** (`20260914234317`). Production **`0057`** is Abel/PM gated and is **not** claimed applied. Polish 0056 file on `main` is not overwritten. |
...
| Gap-edit 2 change | Individual enrichment **writes**, document upload, restriction set/lift, emergency CRUD, and Scope C links use the same chain (`linkGuestAccount` / `unlinkGuestAccount`). **Flag Abel** if a new entitlement type, RLS role, or hard-block RPC is proposed |
...
2. Rekik **tech-plan approval** (issue [#109](https://github.com/NORUDEVGIT/NORU/issues/109)) — **DONE** (CONDITIONAL PLAN APPROVAL satisfied; Linking REQUIRED already on the amended TIP; coding AUTHORIZED)
3. Implementation against this §9 contract (extend Individual form / `guest_profiles` / Wave 2 documents / history / Wave 4 `guest_account_links` — do **not** restart; do **not** reopen Waves 1–5 or Gap-edit 1) — **DONE**. PR [#111](https://github.com/NORUDEVGIT/NORU/pull/111) MERGED (enrichment) · PR [#112](https://github.com/NORUDEVGIT/NORU/pull/112) MERGED (staged Create HOLD fix). One Create: guest → upload → links → navigate. After-save-only on create **fails Spec**.
4. Developer QA recorded (`NOT RUN` ≠ `PASS`) — **PARTIAL** (`tsc` PASS; locks **142/142 PASS**; browser **NOT RUN**)
5. Independent QA **PASS** on [#112](https://github.com/NORUDEVGIT/NORU/pull/112) — HOLD **cleared**
6. Human merge of the implementation PRs — **DONE** (#111 2026-09-14T23:07:06Z; #112 2026-09-14T23:30:45Z)
7. Design Execution Report — **DONE** ([issue #109 DER](https://github.com/NORUDEVGIT/NORU/issues/109#issuecomment-5672319503))
8. Docs reconciliation to `main` (code wins) — **this recon**
9. Rekik formal closure **YES** — issue [#109](https://github.com/NORUDEVGIT/NORU/issues/109) **CLOSED** completed 2026-09-14T23:46:37Z (Advisor Outcome Review). Gap-edit 2 is **OPERATIONALLY ACCEPTED** / closed.

Non-prod `0057` **APPLY PASS** on `qcwptraosaudcbjasmul` (version `20260914234317` / `pms_individual_form_enrichment`; 16/16 columns; `guest_emergency_contacts` YES). **Production 0057** remains **Abel / PM gated — NOT applied**. (`0056` is Polish Wave 1 on `main`.)

**Hotel UAT** was **not** required to start Gap-edit 2 engineering. Hotel UAT **is** required for **module COMPLETE**.

This Spec records Gap-edit 2 as **IMPLEMENTED ON MAIN**. ENGINEERING STATUS **COMPLETE / MERGED**. IMPLEMENTATION STATUS **PASS**. HOLD **cleared**. **OPERATIONALLY ACCEPTED** / closed (Rekik formal closure YES / Advisor Outcome Review). Issue [#109](https://github.com/NORUDEVGIT/NORU/issues/109) is **CLOSED** completed 2026-09-14T23:46:37Z (Rekik formal closure YES / Advisor Outcome Review). It does **not** claim Waves 1–5 incomplete. It does **not** rewrite Gap-edit 1. The module is **not** COMPLETE.

Exit product summary: sectioned Individual form (Basic open); first name still required; Contact / Address / collocated Identity upload (Wave 2 store); **staged ID files + staged Linking on New Guest Create** (one action: create → upload → link → navigate; partial-failure retry); edit-after-save Identity / Linking remain; Employment on individual; emergency contacts add/remove; notes + optional source of business; restricted / blacklisted with reason, badges, history, lift; warn on existing stay paths; Wave 4 `guest_account_links` (**AC-GE2-9…13**); Relationships card stays in sync; no KYC / Import / police export / folio routing / guest↔guest family; tenant/auth honesty; non-prod `0057` APPLY PASS (`20260914234317`); production `0057` Abel-gated (NOT applied).
...
| Individual Linking reuses `guest_account_links` | Gap-edit 2 Scope C — no second table; Relationships card remains |
| Emergency contacts prefer a normalized table | Gap-edit 2 — JSON only if Eng records why |
| Gap-edit 2 migration **0057** | Non-prod **APPLY PASS** `qcwptraosaudcbjasmul` / `20260914234317`. **Production Abel-gated NOT applied.** Do **not** claim production applied; do **not** overwrite Polish 0056 file on `main`. |
| Staged-create on New Guest (ID files + Linking before submit) | Gap-edit 2 — after-save-only on create is **not** the product bar; partial failure → retry, never silent full success |
...
| Record Gap-edit 1 as **IMPLEMENTED ON MAIN** (#105) and **OPERATIONALLY ACCEPTED** / closed; AC-GE1-1…24 **PASS**; issue [#103](https://github.com/NORUDEVGIT/NORU/issues/103) **CLOSED** completed | Reopen Waves 1–5, claim the **module** COMPLETE, invent a rate engine / Position-Department / Group-TA enrichment, or apply production 0055 without Abel/PM |
| Record Gap-edit 2 as **IMPLEMENTED ON MAIN** (#111 + #112); ENGINEERING **COMPLETE / MERGED**; IMPLEMENTATION STATUS **PASS**; HOLD **cleared**; **OPERATIONALLY ACCEPTED** / closed; issue [#109](https://github.com/NORUDEVGIT/NORU/issues/109) **CLOSED** completed (§9, Linking **AC-GE2-9…13**, Identity **AC-GE2-14**) | Treat after-save-only create as CURRENT, claim **production `0057` applied**, reopen Waves 1–5 or rewrite Gap-edit 1, invent KYC / Import / police export / folio routing / guest↔guest family / hard-block Res-FO products, claim #109 still OPEN or pending, or claim the module COMPLETE |
...
>
> Gap-edit 1 **IMPLEMENTED ON MAIN** (#105). ENGINEERING STATUS **COMPLETE / MERGED**. IMPLEMENTATION STATUS **PASS**. **OPERATIONALLY ACCEPTED** / closed (Rekik formal closure YES / Advisor Outcome Review). Issue [#103](https://github.com/NORUDEVGIT/NORU/issues/103) **CLOSED** completed 2026-09-14T22:43:40Z.
>
> **Gap-edit 2 IMPLEMENTED ON MAIN** (§9). ENGINEERING STATUS **COMPLETE / MERGED**. IMPLEMENTATION STATUS **PASS**. HOLD **cleared** via [#112](https://github.com/NORUDEVGIT/NORU/pull/112). PRs [#111](https://github.com/NORUDEVGIT/NORU/pull/111) + [#112](https://github.com/NORUDEVGIT/NORU/pull/112) MERGED. Spec [#110](https://github.com/NORUDEVGIT/NORU/pull/110) MERGED. AC-GE2-1…34 **PASS**. Independent QA **PASS** on #112. **OPERATIONALLY ACCEPTED** / closed (Rekik formal closure YES / Advisor Outcome Review). Issue [#109](https://github.com/NORUDEVGIT/NORU/issues/109) **CLOSED** completed 2026-09-14T23:46:37Z (Rekik formal closure YES / Advisor Outcome Review). Does **not** reopen Waves 1–5 or rewrite Gap-edit 1. Non-prod `0057` **APPLY PASS** (`qcwptraosaudcbjasmul` / `20260914234317`). **Production 0057 NOT applied** (Abel-gated). Module COMPLETE remains **NO**.
